...
try {
  ...
  // 2. Startup path (runAllSeeds) writes all six; statuses flip to present.
  const startup = await runAllSeeds({ client: clientRef })
  if (startup.length !== 6 || !startup.every((r) => r.ok && r.wrote)) throw new Error('runAllSeeds failed on fresh wiki')

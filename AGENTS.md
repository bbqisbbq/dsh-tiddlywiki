...
npm run verify:unit   # 剪藏桥（含 IPv6/保留网段 SSRF 回归）/ seed 读失败策略（**gate 断言从注册表派生 gated 清单，v0.23.1**）/ 渲染片段净化器 / 配置密钥遮掩（含 auth.password，v0.20.0）/ **写策略纯函数（type 保留 v0.20.1；时间戳语义 v0.22.10：紧凑格式往返、新建/覆盖/存量、`$:/`、fields 不可覆盖、put() 兜底只补不改写）** / **注入提示词（slim 无参数清单、full 与工具注册表逐项一致、治理约定不丢、**草稿预览与保存路径逐字节一致，v0.22.7**，v0.21.0）** / 微信 adapter 源码级守门（v0.23.0）/ **wechat-setup seed 单一来源守门（v0.23.1：WECHAT_DOCS_TEXT 与 docs/wechat-publish-setup.md 逐字节一致 + 注册表 gate 接线；v0.23.3 起 gate 谓词计数 = 3）** / **公众号发布按钮守门（v0.23.3，`verify-wechat-publish.mjs`：纯函数 + 真跑 spawn 的桩 opencli E2E + 真实 HTTP 路由守卫，17 条）** / **客户端 frame 守门（v0.22.3：空 src 陷阱、活动主题优先级、跨源 try/catch、共享 toast、死标记；v0.22.4 增「生命周期只有一份」去重断言）** / **客户端 frame 内核行为单测（v0.22.4，tsx 直跑源码 + DOM 打桩：空 src 两条重载路径、可见性不显示空 frame、URL 不重复赋值、启动/错误/重试恢复、hash 导航与兜底、dispose 幂等）** / **启动就绪策略（v0.22.5，tsx + 虚拟时钟：44s 冷启动必须判成功、慢启动只警告一次、子进程退出即 exited、硬上限才 timeout、轮询节奏有界、wiki.ts 不得再有 20s 硬编码）** / **快速笔记原生编辑弹窗（v0.22.6，tsx + DOM 打桩：同 URL 有内容不重载、story 被删空必须重载、关闭清 `dataset.loaded`、关闭后重开必重载、空白判据的四种保守边界）** / 客户端 status-cache 合并（tsx 直跑源码）
...
- **发布到公众号按钮**（wechat-publish seed，v0.23.3）：改 `scripts/bundle/wechat-publish/`（`startup.js` / `button.tid` / `icon.svg`）→
  ...
  npm run build
  ```
  版本号只在 `scripts/bundle/versions.mjs` 定义（`WECHAT_PUBLISH_BUNDLE_VERSION`；改了行为要 bump），build 引用它、gen 从 bundle 的 plugin.info 读并写进外层 tiddler（别再写第三处）。按钮契约（三条 `/wechat/*` 路由与 `wechat.*` 配置）见 `docs/wechat-publish-setup.md` §3.6 与 §1 路由表。

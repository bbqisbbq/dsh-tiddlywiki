...
| 项 | 当前值 | 位置 |
|---|---|---|
| **插件版本** | `0.23.4`（v0.23.4 = **三处硬化，全部来自 v0.23.3 上线当天（2026-09-18）暴露的真实事故**。① **提交前拦冲突（数据安全，最重）**：`git pull --rebase --autostash` 的 **autostash 重新应用**冲突后 `rebase --abort` 已无事可 abort → 冲突标记留在工作树 → 60s 的 AutoCommitter 照常 `add -A && commit`，`<<<<<<< Updated upstream` 被**永久提交并推送**进 wiki 的配置 tiddler（`0b19ca6`，09-17），插件随即解析不了自己的配置（微信发布一直关着 + `prompt.extra` 静默失效），而一次设置页保存又把配置写成"残版"。现在 `GitFace.conflictState()` 三重探测（进行中的 **rebase** / 未合并路径 / **工作树残留冲突块**）→ `commit()` 抛 `GitConflictStateError` 拒绝；`/sync` 409+文件名、agent 工具结构化失败、AutoCommitter 去重上报、`pull()` abort 后**复检**（autostash 形态正是 abort 后才暴露）、`/status` 与设置页状态行显示「冲突未解决（N 个文件，已阻止提交）」。⚠️ `MERGE_HEAD`/`CHERRY_PICK_HEAD` **故意不硬拒**——`git commit` 正是收尾它们的动作（**真 git E2E 抓出了这个设计错误**，别再"补全"回去）。② **配置解析失败拒绝写（数据丢失）**：`ConfigStore.set()` 原先在存量 tiddler 解析失败时拿**空缓存**合并 → 一次保存抹掉 `prompt.extra`/`git.remote`/`ui.*`（09-18 13:38 实测）。现在抛 `ConfigUnreadableError` + 不 PUT + `load()` 记 `parseError()` → `/admin/state` 的 `configError` → 设置页顶部红横幅；**瞬时"读"失败仍走合并到缓存**（回归断言守住，别一起拒了）。③ **adapter 版本校验**：`/wechat/ready` 原先只数文件在不在，旧 adapter（不认 `--title-file`、标题仍是必填位置参数）也报"就绪"、点按钮才在 opencli 里报"缺少必填参数"；现在按**定义**判新旧（`weixin-flow.js` 必须 `export function resolveNoteTitle(`、两个入口必须声明 `name: 'titleFile'`——**正则匹配定义，不是字符串出现**，注释里提到不算；子代理第一版夹具正是被 `includes` 骗过），`missing`（不存在）与 `stale`（过旧）语义互斥，按钮预检与 503 分别说「版本过旧」/「缺少发布脚本」。守门 `scripts/verify-conflict-config-guards.mjs`（进 `verify:unit`，23 条：伪 exec 纯逻辑 + **真起临时 git 仓库的冲突 E2E**（真 merge 冲突 / 真残留标记 + **解决后必须能提交**）+ ConfigStore 拒绝写/回归/接线；**已反向验证**：拆 `contentScan` 红 2 条、拆配置拒绝红 2 条）；`verify-wechat-publish.mjs` 17→24 条；bundle 升 `0.2.0`）；v0.23.3 = **TW 笔记工具栏新增「发布到公众号」按钮**（`scripts/bundle/wechat-publish/`，可选功能，与既有 CLI 等价）：TW 按钮 → `POST /dsh-tiddlywiki/wechat/publish` → 宿主 spawn `opencli weixin publish-note` → `GET …/wechat/publish/status` 每 2s 轮询 → 覆盖层显示进度/日志尾部。**只到草稿箱为止**，发表仍人工扫码（沿用 v0.23.2 策略）。三处硬骨头：① **标题不进 argv**——Windows 上 `opencli` 是 `.cmd` shim（Node `spawn('opencli')` 直接 ENOENT），必须 `cmd.exe /d /s /c` + `windowsVerbatimArguments`，于是标题里的 `&`/`|`/`^`/引号就是命令注入面、中文还会被代码页解成乱码 → 宿主写 `<jobDir>/<id>.title`（UTF-8）+ adapter 新增 **`--title-file`**（`weixin-flow.resolveNoteTitle`；位置参数照旧可用、两者都给时位置参数优先）；② **任务化而非同步请求**——命令要跑几十秒到几分钟，POST 立即回 `jobId`，单并发（第二次 409）、15 分钟上限，超时用 `taskkill /pid X /T /F` 杀**整棵进程树**（只 `child.kill()` 会留下孤儿 node 抓着 job 目录 → 实测 `EBUSY`）；③ **可选功能默认不打扰**——`wechat.enabled` 关着时三条 `/wechat/*` 一律 403、按钮插件不进 wiki（新增第 3 个 gated 起步 seed `wechat-publish`）。新增 `src/host/wechat-publish.ts`（`buildPublishInvocation`/`interpretPublishOutcome`/`scanAdapterDir`/`probeOpencli` 纯函数 + `WechatPublishRunner` job registry）、三条路由（`rejectCrossSiteWrite(['POST'])`/`rejectNonRead` + 可选 `x-wechat-publish-token`；`wechat.token` 与其余密钥一样在 `/admin/state` 打码、`********` 回存被丢弃）、config `wechat{enabled,command,token,adapter,dsn}`（`normalizeWechatConfig` 把 tiddler 里的垃圾值回落默认；dsn 默认按 `req.socket.localPort` 推导 loopback）、TW bundle `$:/plugins/dsh/wechat-publish`（`WECHAT_PUBLISH_BUNDLE_VERSION`，build/gen/verify 三件套）；守门 `scripts/verify-wechat-publish.mjs`（进 `verify:unit`，17 条：纯函数 + **真跑 spawn 的「假 opencli」E2E**（.cmd/.sh → node 桩，逐字断言标题只出现在文件里）+ 真实 HTTP 路由守卫（403/405/401/400/409/404）；**已反向验证**：注释掉 `if (!guardWechat(...))` 即红 2 条）+ `scripts/verify-wechat-publish-bundle.mjs`（进 `verify:static`，逐字节/版本/接线）；v0.23.2 = **多图发布命令 `publish-note-imgs` + 发布策略定为「到草稿箱为止」**：正文内嵌 data URI 图先逐张 DataTransfer 上传 CDN（轮询计数到期望值）→ 按 DOM 顺序重写正文 src → insertHTML → 从正文第一张设封面；`selectCoverFromContent` 改轮询修封面假阴性；安装脚本 FILES 收编至 5 个 adapter、守门断言同步；发表（确认弹窗+扫码）由人工完成，`--publish` 保留但 best-effort；新增排错：`stale page identity` 续作、`list_ex`/`appmsgpublish` 核实、eval 必须过滤不可见 toast 残留）；v0.23.1 = **新增 `wechat-setup` seed——「微信公众号发布指南」随插件分发**：v0.23.0 只把「发布元数据规范」做成了 seed，`docs/wechat-publish-setup.md`（387 行安装/换机还原指南）只存在于仓库/npm 包 docs/ 里。新 seed 与 publish-spec 同模式（起步层 + `gate: ctx.wechat === true`，开启「微信发布」才启动写入，笔记「微信公众号发布指南」，`dsh-docs` 标签）；**内容单一来源**——`scripts/gen-seed-wechat-docs.mjs` 从 setup 文档生成 `src/host/seed-wechat-docs.ts`（照 send-to-agent gen 流水线），`scripts/verify-wechat-docs-seed.mjs`（进 verify:unit）守逐字节一致 + gate 接线；`verify-package-contents.mjs` 的 npm pack 改**文件重定向 + 临时缓存目录**（DSH 沙箱禁命名管道 stdio，原 exec 恒 EPERM；npm 默认缓存也在沙箱外），本地终于能跑；seed gate 断言改从注册表派生（selftest / verify-seed-error-policy），新增 gated seed 不再漏改硬编码）；v0.23.0 = **新增「发布到微信公众号」**（`tools/wechat/`，**可选能力、零 host 代码改动**）：wiki 笔记一键发到公众号**草稿箱**，可选点发表。**绕开官方 API 封锁**——2025-07 起官方回收「发布能力」接口对个人主体/未认证账号的权限（`freepublish/submit` 不可用、`draft/add` 常回 48001），改走**浏览器自动化复用已登录后台会话**（opencli + Browser Bridge 扩展），个人号可用、零凭据落盘。数据流：笔记标题 → DSH `POST /render`（TW 自渲染语义 HTML，代码高亮白蹭）→ `wechat-html.js` 补**内联样式** → `weixin-flow.js` 驱动后台填表/insertHTML 写正文/传图/设封面/存草稿。**三个实测坑**（别重踩）：① **必须带 `--trace retain-on-failure`**，不带就对 `mp.weixin.qq.com` 稳定报 `Navigation rejected`（trace 开 5/5 成功、关 8/8 失败；`--site-session ephemeral`/`--keep-tab false`/前后台窗口/重置标签页/重启 daemon 全试过无效，判定 opencli 1.8.7 的 bug）；② **图片上传必须用 DataTransfer 注入**，不用 `page.setFileInput`（依赖 CDP `Page.fileChooserOpened`，本机扩展 v1.0.24 组合下稳定 `not received within 5s`；改在页面上下文直接塞 `input.files` + 派发 change，实测真进 `mmbiz` CDN，代价单图 8MB 上限）；③ **正文必须 `execCommand('insertHTML')`**，`insertText` 把 HTML 当字面文本（内置 `weixin create-draft` 即如此，实测 Markdown 符号原样进库）。**发表需管理员扫码**（微信账号安全机制，不可自动化）→「一键」= 脚本做完排版/上传/填表、人只需扫一次码；默认走**发表**（不推送粉丝、不占群发额度）而非群发。命令 `opencli weixin publish-note "标题" [--cover x.png] [--preview out] [--publish]`（+ `create-article` 从本地 HTML 建草稿）；`node tools/wechat/install-wechat-adapters.mjs` 一键装 adapter 并自检，换机器还原见 `docs/wechat-publish-setup.md`。守门：`verify-package-contents.mjs` +3 断言（发布包必须含 tools/wechat/publish-note.js、install-wechat-adapters.mjs 与 setup 文档）+ `package.json` 的 `files` 补 `tools`——否则 npm 用户拿不到。设计/实测见 `docs/plans/2026-09-17-wechat-publish-design.md`，调研沉淀在 wiki「公众号发布插件调研」）；v0.22.10 = **修「写入丢失 `created`/`modified`，新笔记在 `+[!sort[modified]]` 页面沉底」**：两字段原先在 `CLEAN_SKIP_FIELDS`（假设「服务端会补」——错，`put-tiddler.js` 只 `addTiddler`），缺失被 `sortTiddlers` 的 `fields[sortField] || ""` 当空串、降序即沉底（实测日记 175/175）。现在 `buildWriteTiddler()` 统一写：新建 = 都取当前时刻，覆盖 = 保留原 `created`、刷新 `modified`；格式 `formatTiddlerDate()`（TW 17 位紧凑 UTC，别用 ISO）；仍在 `RESERVED_TIDDLER_FIELDS` 但不再被 CLEAN_SKIP 丢；`TiddlyWebClient.put()` 加 `ensureTiddlerTimestamps()` 兜底（只补缺、不改写），rename/restore/草稿/配置一并统一；守门 verify-write-policy +7 纯函数、verify-tools +5 E2E（含真实 `!sort[modified]` 顺序）+ selftest 配置 created 保留；v0.22.9 = **修「TW 编辑者署名被静默清空」**（用户实测报障：控制面板「编辑者署名」保存不了、刷新就没）：匿名 loopback spawn 分支**没传 `anon-username`** → `get-status.js:21` 回落到 `""` → TiddlyWeb adaptor 用 `isLoggedIn = json.username !== "GUEST"`（`tiddlywebadaptor.js:93`）把 `""` 判成**已登录** → `syncer.js:281-283` 每次页面加载都把 `$:/status/UserName` 覆写成空串；一旦 `$:/config/SyncFilter` 放行 `UserName`（本仓库 wiki 现状），空串还会落盘进 git → 真实、可追溯的数据丢失。现在匿名分支传 `anon-username=GUEST`（`wiki.ts` 导出常量 `ANON_USERNAME`）：`isLoggedIn=false` → syncer 不再写 `UserName`。⚠️ `GUEST` 是**承重哨兵**，填任何人名都仍算已登录（只会把署名覆盖成人名而非空串）。**拖到今天的原因就是 `wiki.ts` 那句错误注释**（「anon-username/readers/writers … verified to 401 every request」把两个独立参数混为一谈，已改正）：实测只有 `readers`/`writers` 会关匿名访问（`server.js:63-66` 显式 `readers` 让 `(anon)` 不再是 principal → `isAuthorized()` false → 401 `'undefined' is not authorized`），`anon-username` **单独**传完全安全（`/status` 200 / GET 200 / PUT 204）。副作用（外观级）：`IsLoggedIn` 恒 `no` → SyncerDropdown 多一个无用 Login 按钮；`generateDraftTitle` 从 `Title` 切到 `Attribution` 形态（内核既有行为，与 `/edit` 草稿路由**无关**——后者按 `draft.of` 字段匹配，不依赖标题字面量）。影响面：默认 SyncFilter 的用户**不会**丢盘（`$:/status/` 被默认过滤器整体排除，空串走不到落盘），实际受数据丢失影响的只有**已放行 `UserName` 的人**。守门：新增 `scripts/verify-anon-username.mjs`（进 `verify:e2e`，真实 TW 子进程：`ANON_USERNAME` 必须字面量 GUEST / `/status` 必须回该哨兵 / 落盘署名不得被覆盖 / 匿名仍可正常写条目 / auth 模式不受影响且对匿名请求仍 401 + 源码级断言「旧误导注释必须已删、鉴权分支不得实际传 anon-username」；**已反向验证**：去掉那行即红 3 条）；v0.22.8 = **第六轮代码审计修复**（host + client 各一份只读审计，逐条核实后修 10 处真缺陷；用户要求「消除重复/死代码以及其他可修复缺陷」）：① **P0 数据丢失——`/edit` 覆盖用户在 TW 里未保存的草稿**：v0.20.0 只修掉「另起时间戳标题」，但仍会把 `draftText`（调用方文本或笔记已保存正文）PUT 进**刚复用**的那个草稿；触发路径极常见（快速笔记标题精确到分钟：打开→写→关→同一分钟再开＝标题相同、调用方文本为空 → 空白覆盖）。现在 `draftExists` 标记复用，「新建的草稿才由我们填充，复用的只有调用方显式给了文本才写」；② **P0 `fetchJson` 丢弃调用方超时**（`{...init, signal: timeout(15s)}` 里后者恒胜）→ 知识库切换/恢复默认的 120s 被 15s 掐断、宿主其实切成功了却弹「切换失败」（大 wiki 冷启动实测 44s+）；改 `init?.signal ?? timeout(15s)`；③ **净化器越界实体崩服务**：`decodeEntities` 只判 `code > 0`，`&#1114112;`（0x110000）让 `String.fromCodePoint` 抛 `RangeError`，而 `sanitizeTwFragment` 没有 try → 一条正文即可让 `POST /render` 502、打坏工具卡与会话 Tab；改 `codePointToChar()` 显式挡 > 0x10FFFF；④ **快速笔记「幽灵草稿」**：`saveDone()`/「丢弃」重置时 `editor.setValue('')` 触发防抖，500ms 后把「空正文 + 新时间戳标题」写成草稿 → 下次打开误报「已恢复未保存草稿」且跳过默认 tag（「已丢弃草稿」也会复活）；现在 `resetForNewNote()` 取消防抖 + 记录已定型签名，`flushDraft` 再用 `autoTitle` 识别「只有自动标题、正文为空」；⑤ **`/recent` 丢掉 `since`**：工具与回复流卡片都发 since，路由只读 limit → 模型看到过滤结果、人看到未过滤卡片（与 v0.20.0 修过的 `/search` 同类漂移）；现在透传并回显；⑥ `doRefresh` await 后不复查可见性 → 隐藏面板会重新起一轮 30×1.5s `/status` 轮询；⑦ `/tw` 代理补显式方法白名单（与 `/api` 一致，TRACE 等不再转发）；⑧ `/render` 的 404 改结构化 `RenderNotFoundError`（原按错误消息文本正则，改词即把 notFound 变 502）；⑨ `/get` 改用共享 `isBlockedProxyTitle()`（原硬编码字面量，新增密钥命名空间会静默漏）；⑩ 侧边栏显示名保存后即时生效（原只挂载读一次，改设置要刷新整页）。**去重/死代码**：`render-fetch.ts`（两处 `POST /render` 合一）、`describeSyncResult()`（FAB 与设置页同步回执合一，此前已漂移）、`ui-config.ts` 承担全部 `ui.*` 投影（FAB/note-widget 不再各写一份）、`formatLocalMinute()`（host 两处 `YYYY-MM-DD HH:mm`）、`DOC_NOTE_DSH_DOCS_TAG` 改为 `DSH_DOCS_TAG` 别名（原为两份同值字面量）；删 `ensureTwWebHost()`（生产零调用，与 `tw-web-host` seed 重复）、`TiddlyWebClient.listTags()`（仅 selftest 用且与工具 clamp 重复）、`clampLimit`/`applyCustomFields`/`finalTagsForWrite` 转模块私有，十余个只在自身模块内使用的 `export` 撤销，`SyncStateView.label`/`lastSync` 等计算了却从不显示的字段删除；守门：`verify-render-sanitizer.mjs` 新增越界实体断言、`verify-frame-guards.mjs` 新增 3 条（fetchJson signal / doRefresh 可见性 / 幽灵草稿）、`verify-audit-fixes.mjs` 新增 3 条（草稿不被空文本覆盖、显式文本仍写入、`/recent` since 透传），**草稿覆盖那条已反向验证**（还原修复即红）；v0.22.7 = **设置页「预览注入文本」改为按表单当前值渲染**（用户提问「两种形态差别？为什么选了不同形态后预览显示一样？」）：差别其实只在 intro 一段（slim 1735 字符一句能力概览 / full 2504 字符的注册表生成参数索引，三块治理约定完全相同），但旧预览只发 `GET /admin/prompt` 读**已保存**配置，而下拉框的值只活在浏览器 DOM → 不保存就预览＝逐字节相同的旧文本。现在 `prompt.ts` 新增 `normalizePromptPreview()`（白名单 4 字段，未知键/错类型丢弃回落内置默认）+ `describePrompt()`（**与 `applyPrompt()` 同一份拼装**），`POST /admin/prompt` 收草稿渲染全文且**零副作用**（不落盘、不触发 `onConfigChanged`；同源守卫 + 畸形 JSON 400），设置页按钮改名「预览注入文本（按表单当前值）」并在有未保存改动时标注；守门 `verify-prompt.mjs` 3 条纯函数断言 + `verify-seeds-admin.mjs` 第 8 段 7 条草稿断言（原「POST 必须 405」已删）；v0.22.6 = **修「快速笔记」两处用户实测报障**：① **native 原生编辑弹窗删掉笔记后变永久白板**——TW「删除」走 `tm-delete-tiddler`（删原 tiddler + 草稿 + `removeTitleFromStory` 清空 story），而主机重建的草稿标题常与删除前相同（时间戳精确到分钟 + canonical `Draft of "…"` 复用）→ URL 不变 → 旧的「只在 URL 变化时赋 `src`」守卫不重载 → 弹窗只剩白板、再点按钮也回不来；现在 `isEditorPopupBlank()`（读同源 iframe 的 `.tc-story-river`，没有 `.tc-tiddler-frame` 即判空；跨源/loading/读不到文档一律不判空）触发**强制重载**，`closeEditorPopup()` 清 `dataset.loaded`（关闭再打开 = 重新载入），dock 与 `openNative()` 的「已打开」幂等守卫都排除空弹窗；守门 `scripts/verify-editor-popup.mjs`（11 条 DOM 级行为断言，反向验证过）；② **card 模式底部操作条按钮文字被 flex 收缩压成两行**（实测每个按钮 `textRects=2`、高 46px）：现在按钮 `white-space:nowrap` + `flex:0 0 auto`，装不下时整个右组换行并贴右，「Ctrl+Enter」提示可收缩让位；v0.22.5 = **修「大知识库冷启动被误判为启动失败」**（`wiki server did not become ready in time`）：5099 条目 / 26MB wiki 冷缓存实测要 ~44s 才 `Serving on`，旧的单一 20s 硬超时既抛错中断整条启动管线（配置/seed/剪藏桥全跳过），又把 `health='failed'` 粘死（超时后无人复探）。现在 `ready-policy.ts` 三段策略：软窗口（默认 60s，`startup.readyTimeoutMs` 可调 5s–600s）只警告并继续等 → 硬上限 3× 才判失败 → 失败不杀子进程而是挂有界的「迟到就绪」复探（TW 真起来立刻转 running 并清错误）+ `verify-ready-policy.mjs`；v0.22.4 = **客户端 TW frame 生命周期去重**：`panel.ts` 与 `rightbar` 各带一份的 showError/showStarting/showFrame/fallbackLoad/doRefresh 收敛为 `tw-frame.ts` 的 `createTwFrameSurface(skin)` **唯一实现**（v0.22.3 的空 `src` bug 正是两份漂移的产物：守卫只补到其中一份），panel 退回「皮肤 + 惰性构建点 + 可见性」，并新增 `verify-frame-surface.mjs`（16 条 DOM 级行为断言，含反向验证）；v0.22.3 = 第五轮审计的**客户端**修复（host 侧逐条对照零缺陷）：iframe 空 `src` 陷阱（FAB 重载 / hash 兜底加载改用 `dataset.loaded`）、`/admin/state` 回传真实活动主题 `info.themeActive`、`applyToFrame` 跨源 try/catch、卸载守卫与死标记清理 + `verify-frame-guards.mjs`；v0.22.2 = 修「主题汇总页·模板」seed 内容重复：删掉裸 `<$list>` 可选段，模板只留一个列表；v0.22.1 = 首页快速笔记标题日期 token 修复 + gen-seed-home 转义修复；v0.22.0 = 运行时切换知识库 + seed 内容哈希更新检测 + flush 真正排干；v0.21.0 = 注入提示词精简+可配置） | `package.json` `version`（三处版本一致性由 `scripts/verify-version-consistency.mjs` 守门） |
...
| **Seed 注册表（13 项，三层）** | 核心（自动写、不可移除）：`send-to-agent`、`render-route`、`tw-web-host`；起步（首次安装默认写、可移除）：`doc-note`、`starter-docs`、**gated 三项（v0.23.0/0.23.1/0.23.3，`startup: true` + `gate: (ctx) => ctx.wechat === true`——**只在设置页开启「微信发布」(`wechat.enabled`) 时才写**，`gate` 只作用于启动路径、设置页手动「初始化」不受它约束）：`publish-spec`（v0.23.0「发布元数据规范」——开启时提示词里有 `[[发布元数据规范]]` 指针，文档不落地就是死链）、`wechat-setup`（v0.23.1「微信公众号发布指南」——**逐字节等于 `docs/wechat-publish-setup.md`**，由 `scripts/gen-seed-wechat-docs.mjs` 生成，`scripts/verify-wechat-docs-seed.mjs` 守逐字节一致）与 `wechat-publish`（v0.23.3「发布到公众号」工具栏按钮插件 `$:/plugins/dsh/wechat-publish`——由 `scripts/gen-seed-wechat-publish.mjs` 生成，`scripts/verify-wechat-publish-bundle.mjs` 守逐字节一致 + 版本 + 接线；三项同进同退，gate 谓词在 seeds.ts 里恰好出现 3 次，`verify-wechat-docs-seed.mjs` 数它）**；可选（手动）：`home-index`、`all-articles`、`ui-styles`、`menubar-theme`、`clip-bridge`（剪藏桥使用说明，真功能在 `clip-bridge.ts` 运行时代码里）。文档类内容统一打 `dsh-docs` 标签（进首页「📚 插件文档」栏）。**v0.22.0 起**：有 `markerTitle` + `content` 的 seed 会在标记里记**内容哈希**（`{version,hashes,at}`）以支持「有更新/本地已修改」检测；`doc-note` 的正文**由工具注册表生成**（`docNoteText(tools)`，不再手抄工具清单） | `src/host/seeds.ts` 的 `SEED_DEFS` + `src/host/seed-util.ts`（`hashText`/`parseSeedMarker`/`readSeedMarker`/`writeSeedMarker`） |
| **注入提示词** | `buildPromptText()`（name `dsh-tiddlywiki`，order 100，**默认 `slim`**）：工具能力一句话 + 可点击链接格式 / 写入与并发纪律（先 `get` 拿 `modified`→`expectedModified`；`attach` 同名默认拒绝；不传 tags 保留原标签/字段/类型）/ 同步纪律三条 + pull 冲突处理 / 标签约定（`agent-written`/`human-edited`/workspace tag）/ 内容类型约定 / 想法沉淀（`todo`）约定。`full` 形态另附**由 `tiddlywikiToolSummary()` 实时生成**的工具参数索引（禁止再手抄 schema）。配置 `prompt{enabled,mode,extra,override}`；用户文本里的 `{{…}}` 必须转义。**可选功能不打扰（v0.23.0）**：`buildPromptText({wechat})` 默认 **false**，此时不输出任何发布相关文字（微信发布是可选 + 需额外安装的，默认关者的每个会话都多一行纯属噪音）；`wechat.enabled` 打开后才追加一行（~61 字符），`normalizePromptPreview`/`describePrompt` 也认这个字段，**设置页预览按表单当前值**所以勾选后立刻可见。**预览（v0.22.7）**：`describePrompt()` 是预览与 `applyPrompt()` 共用的唯一拼装——`GET /admin/prompt` = 已保存/正在注入，`POST /admin/prompt` = 表单草稿（不落盘；设置页默认走它） | `src/host/prompt.ts`（v0.21.0 从 `src/index.ts` 抽出） |
| **配置项** | `wikiRoot`/`wiki`（**v0.22.0 起只是默认值**：运行中实际位置 = 指针文件 > cordis config > 内置默认）/`port`/`git{autoCommit,debounceMs,remote,branch}`/`note{tag}`/`startup{readyTimeoutMs}`（**v0.22.5**：软就绪窗口，默认 60000、夹 5s–600s，硬上限 = 3×，对下次启动/重启生效）/`prompt{enabled,mode,extra,override}`/`bridge{enabled,port,token,tag}`/**`wechat{enabled,command,token,adapter,dsn}`（v0.23.0 起 `enabled` 默认 `false`——关时不注入发布提示词、不写三个 gated seed（元数据规范/换机指南/发布按钮）且三条 `/wechat/*` 路由一律 403；**v0.23.3** 新增 `command`（CLI 路径，默认 `opencli`）/`adapter`（`publish-note`|`publish-note-imgs`）/`token`（非空时要求 `x-wechat-publish-token`，与其它密钥一样在 `/admin/state` 打码）/`dsn`（adapter 回连基址，空 = 按 `req.socket.localPort` 推导 loopback）；`normalizeWechatConfig()` 把 tiddler 里的垃圾值回落默认，绝不带进命令行）**/`ui{showQuickNote,showQuickNoteDock,quickNoteMode,sidebarLabel,showPanelStatus,showSyncButton,followDshTheme,darkPalette,sendToAgent{enabled,endpoint,token},allArticles{pageSize},tabLabel,showSessionTab,showRightbarTab}`/`uiLanguage`/`auth{username,password}` | `src/host/config.ts` |
| **DSH 路由** | `/status` `/note` `/edit` `/tags` `/recent` `/get` `/search` `/render` `/sync` `/upload` `/restart` `/session/summary` `/agent/sessions` `/agent/modes` `/agent/send` `/agent/create`／**v0.23.3 公众号发布**（可选功能关时一律 403）：`/wechat/ready`（GET 预检 opencli + adapter）、`/wechat/publish`（POST 起任务，单并发 409）、`/wechat/publish/status`（GET 轮询，`?id=`） `/api/*` `/tw/*`；admin：`/admin/state` `/admin/prompt`（**v0.22.7**：GET 已保存／POST 草稿预览） `/admin/info` `/admin/config` `/admin/restart` `/admin/seeds` `/admin/seeds/run` `/admin/seeds/remove`／**v0.22.0 知识库位置**：`/admin/wiki/location`（GET 读）、`/admin/wiki/switch`（POST 切）、`/admin/wiki/reset`（POST 恢复默认） | `src/host/routes.ts` + `src/host/admin.ts` |
...
├── host/
│   ├── wiki.ts         # WikiServer：spawn/kill/自愈/端口探测/就绪轮询（策略在 ready-policy.ts）+ 迟到就绪复探；TW_PROXY_PATH 同源代理路径
│   ├── ready-policy.ts # TW 启动就绪策略（v0.22.5）：normalizeReadyTimeoutMs（默认 60s / 夹 5s–600s）/ readyHardTimeoutMs（3×）/ awaitReady（软窗口只警告、硬上限才 timeout；probe/isAlive/now/sleep 全注入，供虚拟时钟单测）
...
│   ├── write-policy.ts # 共享写策略（v0.19.1）：cleanTiddler/buildWriteTiddler/stampTiddlerTimes/assertNoConflict/flattenTiddlerFields —— agent 工具与 /note /edit 路由共用
...
│   ├── text-util.ts    # 共享文本/日期助手：snippetOf（v0.20.0）+ formatLocalMinute（v0.22.8，routes 与 session-summary 的 YYYY-MM-DD HH:mm 合一）
│   ├── prompt.ts       # 注入提示词（v0.21.0）：slim（默认）/full 两种形态 + extra/override；full 的参数索引由 tiddlywikiToolSummary() 生成；escapePromptBraces 防 DSH 变量装配失败；**v0.22.7 草稿预览**：normalizePromptPreview（白名单 4 字段）+ describePrompt（与 applyPrompt 同一份拼装）
│   ├── wechat-publish.ts # 公众号发布执行器（v0.23.3，可选功能「发布到公众号」按钮的后端）：buildPublishInvocation（win32 走 cmd.exe + windowsVerbatimArguments；标题只经 `--title-file`，不进 argv）/ buildVersionInvocation / normalizeWechatConfig / isSafeCliValue / isSafeDsn / interpretPublishOutcome / scanAdapterDir / probeOpencli（`opencli --version`，超时即 false）+ **WechatPublishRunner**（单并发 job registry：15 分钟上限、超时 `taskkill /T` 杀整棵树、输出有界、dispose 随插件）
...
│   ├── ui-config.ts    # 共享 ui.* 投影（v0.19.5，v0.22.8 扩到全部 FAB 开关）+ invalidateUiConfig/subscribeUiConfig（设置页保存后通知侧边栏等即时重读）
│   ├── render-fetch.ts # **唯一**的客户端 POST /render 调用（v0.22.8）：tool-views 与 session-summary 原先各一份；CSRF 头是承重的
│   ├── panel.ts        # 中央列 TW 覆盖层：**只管布局**（rect 钉住 / 惰性构建 / 共存 chrome / 互斥）；iframe 生命周期全交给 tw-frame（v0.22.4 去重后从 469 行降到 280 行）
│   ├── rightbar-tab.ts # 右侧栏 TW tab（v0.16.21）：type 注册 + guide 入口 + React body（TW iframe）/互斥
│   ├── tw-frame.ts     # **TW frame 内核（v0.22.4 起唯一实现）**：`createTwFrameSurface(skin)` 拥有 lazy-load / status 轮询 / 错误与启动态 / 主题同步 / FAB 重载 / hash 导航 / dispose；`createTwFrameController(host, signal)` 是它的 rightbar 薄包装；另含互斥事件、共享 chip 标签与 live-frame 链接路由注册表。⚠️ 生命周期行为只能加在这里（§8）
...
tools/wechat/           # **发布到微信公众号**（v0.23.0，可选能力，不进 host 运行时）：wechat-html.js（语义 HTML → 内联样式装饰器，主题表在此）/ weixin-flow.js（浏览器流程：填表/insertHTML/传图/封面/存草稿/发表）/ create-article.js（本地 HTML → 草稿）/ publish-note.js（**主入口**：TW 笔记 → 草稿，含 --preview、发布前状态检查）/ install-wechat-adapters.mjs（一键装到 ~/.opencli/clis/weixin/ 并自检）/ backfill-publish-state.mjs（存量回填 pub-* 字段，默认 dry-run）/ seed-publish-spec-now.mts（把「发布元数据规范」立即写进当前 wiki）。⚠️ 这几个 .js 是**用户态的 opencli adapter**，不进 lib/、不被 typecheck 覆盖，由 `scripts/verify-wechat-adapters.mjs` 守门（源码级，不进 npm scripts 之外的地方——已挂进 verify:unit）
docs/seed-initialization.md  # seed 机制详解（权威）
docs/wechat-publish-setup.md # 换机器还原公众号发布能力（含 opencli/扩展安装、扫码、排错表）
docs/plans/             # 设计文档（含 2026-09-17 公众号发布设计 + 全部实测依据）
...
npm run verify:static # send-to-agent bundle 逐字节 / render bundle 逐字节 / **发布到公众号 bundle 逐字节（v0.23.3：bundle↔seed 常量、内层 plugin.info 版本 === WECHAT_PUBLISH_BUNDLE_VERSION、外层 tiddler plugin-type、LF/无 BOM、三份源件与 bundle 逐字节）** / 发布包内容 / 版本一致性 / 静态常量（TEXT_LIST_FILTER 长度预算）
npm run verify:unit   # 剪藏桥（含 IPv6/保留网段 SSRF 回归）/ seed 读失败策略（**gate 断言从注册表派生 gated 清单，v0.23.1**）/ 渲染片段净化器 / 配置密钥遮掩（含 auth.password，v0.20.0）/ **写策略纯函数（type 保留 v0.20.1；时间戳语义 v0.22.10：紧凑格式往返、新建/覆盖/存量、`$:/`、fields 不可覆盖、put() 兜底只补不改写）** / **注入提示词（slim 无参数清单、full 与工具注册表逐项一致、治理约定不丢、**草稿预览与保存路径逐字节一致，v0.22.7**，v0.21.0）** / **冲突与配置守卫（v0.23.4，`verify-conflict-config-guards.mjs`：23 条——伪 exec 纯逻辑 + 真起临时 git 仓库的冲突 E2E + ConfigStore 拒绝写/回归/接线）** / 微信 adapter 源码级守门（v0.23.0）/ **wechat-setup seed 单一来源守门（v0.23.1：WECHAT_DOCS_TEXT 与 docs/wechat-publish-setup.md 逐字节一致 + 注册表 gate 接线；v0.23.3 起 gate 谓词计数 = 3）** / **公众号发布按钮守门（v0.23.3，`verify-wechat-publish.mjs`：纯函数 + 真跑 spawn 的桩 opencli E2E + 真实 HTTP 路由守卫，17 条）** / **客户端 frame 守门（v0.22.3：空 src 陷阱、活动主题优先级、跨源 try/catch、共享 toast、死标记；v0.22.4 增「生命周期只有一份」去重断言）** / **客户端 frame 内核行为单测（v0.22.4，tsx 直跑源码 + DOM 打桩：空 src 两条重载路径、可见性不显示空 frame、URL 不重复赋值、启动/错误/重试恢复、hash 导航与兜底、dispose 幂等）** / **启动就绪策略（v0.22.5，tsx + 虚拟时钟：44s 冷启动必须判成功、慢启动只警告一次、子进程退出即 exited、硬上限才 timeout、轮询节奏有界、wiki.ts 不得再有 20s 硬编码）** / **快速笔记原生编辑弹窗（v0.22.6，tsx + DOM 打桩：同 URL 有内容不重载、story 被删空必须重载、关闭清 `dataset.loaded`、关闭后重开必重载、空白判据的四种保守边界）** / 客户端 status-cache 合并（tsx 直跑源码）
npm run verify:e2e    # auth 模式 / **verify-anon-username（v0.22.9：真起 TW，`/status` 必须回 GUEST 哨兵、落盘署名不被覆盖、匿名仍可写、auth 模式不受影响且匿名仍 401）** / 工具层（含 v0.22.10 的 5 条时间戳 E2E：新建落盘、覆盖保 created、真实 `!sort[modified]` 顺序、append/rename、fields 不可覆盖） / **审计回归 verify-audit-fixes**（attach 覆盖保护 / 草稿类型 / 回收站索引 / delete 并发 / rename 部分失败 / 汇总未探测 / batch_put 并发顺序）/ git 冲突解决 / 崩溃自愈+并发写 / seed 两个 E2E / **verify-wiki-switch（v0.22.0：真起两个 wiki，切换 / 回滚 / 指针文件 / 非法输入）**
...
- **微信公众号发布指南**（wechat-setup seed，v0.23.1）：改 `docs/wechat-publish-setup.md`（唯一手编源）→ `node scripts/gen-seed-wechat-docs.mjs` → `npm run build`；`scripts/verify-wechat-docs-seed.mjs` 断言 seed 常量与文档逐字节一致（忘跑 gen 即红）。
- **发布到公众号按钮**（wechat-publish seed，v0.23.3）：改 `scripts/bundle/wechat-publish/`（`startup.js` / `button.tid` / `icon.svg`）→
  ```bash
  node scripts/build-wechat-publish-bundle.mjs                                            # → scripts/bundle/wechat-publish.bundle.json
  node scripts/gen-seed-wechat-publish.mjs scripts/bundle/wechat-publish.bundle.json src/host/seed-wechat-publish.ts
  node scripts/verify-wechat-publish-bundle.mjs                                           # 逐字节 + 版本 + 接线
  npm run build
  ```
  版本号只在 `scripts/bundle/versions.mjs` 定义（`WECHAT_PUBLISH_BUNDLE_VERSION`；改了行为要 bump——v0.23.4 升 `0.2.0`：预检认 `adapters.stale`，旧版 adapter 说「版本过旧」而不是「缺少发布脚本」），build 引用它、gen 从 bundle 的 plugin.info 读并写进外层 tiddler（别再写第三处）。按钮契约（三条 `/wechat/*` 路由与 `wechat.*` 配置）见 `docs/wechat-publish-setup.md` §3.6 与 §1 路由表。
...
- **TW 启动就绪策略（v0.22.5）**：`WikiServer` 的就绪判定分三段——**软窗口**（`readyTimeoutMs`，默认 60s，`startup.readyTimeoutMs` 可调、夹 5s–600s）之内安静等待；超过它只写一条 `slow start … still loading` 日志并**继续等**（`health` 保持 `starting`，**不粘 error**）；**硬上限 = 3×** 才判失败，且失败**不杀子进程**，而是挂一个**有界（10 分钟）+ `unref` 的「迟到就绪」复探**，TW 真应答时立刻转 `running` 并清错误（`stop()` / 下一次 `start()` 会取消它）。纯策略在 `src/host/ready-policy.ts`（`awaitReady` 的 probe/isAlive/now/sleep 全部注入，便于虚拟时钟单测），`WikiServer.setReadyTimeout()` 由 `applyServerTuning()` 在启动后与 `/admin/config` 保存后各调一次（对下一次启动/重启生效）。

#### 公众号发布按钮（v0.23.3，可选功能）
- TW 工具栏按钮（`$:/plugins/dsh/wechat-publish`，seed `wechat-publish` 写入）→ `POST /wechat/publish` → 宿主 `WechatPublishRunner` spawn `opencli weixin publish-note` → TW 每 2s 轮询 `/wechat/publish/status`。**只到草稿箱**，发表人工扫码。
- **标题不进 argv**：宿主写 `<jobDir>/<id>.title`（`$DSH_HOME/dsh-tiddlywiki/wechat-publish/`，UTF-8），argv 里只有 `--title-file <我们生成的路径>` + 白名单 adapter + 由 `req.socket.localPort` 推导的 loopback dsn；`--trace retain-on-failure` 写死。config `wechat{enabled,command,token,adapter,dsn}`，`token` 走 `x-wechat-publish-token` 且与其它密钥一样打码。
- 单并发（第二次 409）、15 分钟上限、超时/销毁时 `taskkill /T` 杀整棵树；`dispose()` 已挂进 `disposers`。就绪探测每次真跑 `opencli --version`（按钮预检与起任务前各一次，故意不缓存）。
- 改这个按钮 = 改 `scripts/bundle/wechat-publish/` 后走 §4 的三件套；host 侧逻辑在 `src/host/wechat-publish.ts`。

### 提交/配置守卫（v0.23.4，来自真实事故）

- **`git.ts`**：`GitFace.conflictState(dir, {contentScan?})` → `{conflicted, reason, files}`；`commit()` 在 `contentScan: true` 下**拒绝**提交并抛 `GitConflictStateError(reason, files)`（`describeConflict()` 统一措辞）。三条探测：`.git` 里的 **rebase** 引用（`REBASE_HEAD`）/ `diff --diff-filter=U` 未合并路径 / 变更文件里的 `<<<<<<< `、`>>>>>>> ` 行（正则 `^(?:<{7}|>{7}) `，**只认带标签的形态**，Markdown 分隔线不算）。⚠️ **`MERGE_HEAD`/`CHERRY_PICK_HEAD`/`REVERT_HEAD` 故意不在硬拒列表**：`git commit` 正是收尾它们的动作，硬拒会让"已解决的 merge"永久提交不了（真 git E2E 抓出来的）。`status()` 在 dirty 时才做内容扫描并带 `conflict?`；`pull()` 在 `rebase --abort` **之后复检**（autostash 重新应用冲突正是 abort 后才暴露）。`AutoCommitter` 捕获该错误并按签名**只上报一次**，成功提交后重置。
- **`config.ts`**：`ConfigStore.parseError()` / `ConfigUnreadableError` / `describeUnreadableConfig()`。`load()` 解析失败 → 保留缓存 + 记错误；`set()` 在"tiddler 存在但解析失败"时**抛错且不 PUT**（拿空缓存合并＝抹掉用户其它设置）；**瞬时"读"失败仍合并到缓存**（回归断言守住）。
- **接线**：`/sync` 与 `tiddlywiki_git_sync`/`git_resolve` 捕获 `GitConflictStateError` → 409/结构化失败（带 `conflictFiles`）；`/admin/state` 回 `configError`、`/admin/config` 特判 `ConfigUnreadableError` → 409；设置页渲染红色横幅 + 状态行「冲突未解决」。
- **`wechat-publish.ts` 的 `stale`**：`WECHAT_FRESHNESS_MATCHERS`（`export function resolveNoteTitle(` / `name: 'titleFile'`）判 adapter 新旧，`missing`（不存在）与 `stale`（过旧）**互斥**；读文件失败＝过旧。
...
- **写入保留字段（v0.19.0 / 抽成共享策略 v0.19.1，数据安全）**：写策略住在 **`src/host/write-policy.ts`**（`cleanTiddler`/`buildWriteTiddler`/`stampTiddlerTimes`/`assertNoConflict`/`flattenTiddlerFields`），**agent 工具与 `/note`、`/edit` 人类路由共用同一份实现**。`buildWriteTiddler()` 以**已有条目为基底**构造 PUT：不传 `tags` 就保留原标签/自定义字段/内容类型，显式传 `tags` 才整体替换；`agentTag: false`（人类路由）不补 `agent-written`。⚠️ 单条 GET 的自定义字段是**嵌套在 `fields` 里**的（`get-tiddler.js` 把 knownFields 之外的字段收进 `fields`），必须摊平，否则 put/append/rename/trash 会静默丢字段，`tiddlywiki_get` 也会渲染成 `fields=[object Object]`。
- **时间戳由插件自己写（v0.22.10，别再假设「服务端会补」）**：`created`/`modified` 现在**不在** `CLEAN_SKIP_FIELDS` 里，由 `buildWriteTiddler()` 末尾的 `stampTiddlerTimes()` 写：**新建** ⇒ 两者 = 当前时刻；**覆盖** ⇒ 保留基底 `created`、刷新 `modified`；基底无 `created`（迁移存量）⇒ 补当前时刻。值用 `formatTiddlerDate()`（TW 的 17 位紧凑 UTC `YYYYMMDDhhmmssSSS`，与 `$tw.utils.stringifyDate()` 逐字节一致；**别用 ISO**，否则磁盘 `.tid`/`.meta` 与 TW 编辑器写出的不一致）。两者仍在 `RESERVED_TIDDLER_FIELDS`（`fields` 不得覆盖）。`TiddlyWebClient.put()` 里还有 `ensureTiddlerTimestamps()` 兜底（**只补缺、绝不改写**），覆盖剪藏桥/seed/配置/汇总等手拼 body 的路径。守门：`verify-write-policy.mjs`（纯函数）+ `verify-tools.mjs` 的 5 条 E2E（含真实 `!sort[modified]` 顺序）+ selftest 配置 created 保留。
...
- **预览接口（v0.22.7 起分两态）**：`GET /admin/prompt` = **已保存/正在注入**的文本；`POST /admin/prompt`（body `{enabled,mode,extra,override}`）= **表单草稿**的文本，经 `normalizePromptPreview()` 白名单化后交给 `describePrompt()`，**只读、不落盘**（设置页默认走 POST，切换形态立刻可见）。两者都回 `{ok, draft, enabled, mode, length, text}`，CSRF 硬化，畸形 JSON 400。⚠️ 别再让预览去读已保存配置——那正是 v0.22.7 修掉的「切换形态后预览没变化」。
- 守门：`scripts/verify-prompt.mjs`（进 `verify:unit`）——slim 不得含参数清单、长度 ≤1800；full 的每个工具与每个参数都必须在其目录行出现；两种形态都必须包含全部治理约定块；`enabled:false` 返回空串；`extra`/`override` 组合语义；`{{` 转义；**v0.22.7 草稿预览**（`normalizePromptPreview` 白名单、`describePrompt` 与 `buildPromptText` 逐字节一致、两种形态在草稿里必须不同）。E2E 在 `verify-seeds-admin.mjs` 第 8 段（GET `/admin/prompt` 的内容与模式、POST `/admin/config` 触发 `onConfigChanged`、**POST 草稿预览 7 条**：形态可辨/extra 追加/`enabled:false` 为空/GET 不变且未触发 onConfigChanged/垃圾字段回落/畸形 JSON 400/跨站 403）。
- ⚠️ 顺序约束：`applyPrompt()` 必须在 `registerTiddlywikiTools()` **之后**首次调用，否则 `full` 目录是空的。

### Seed 机制（src/host/seeds.ts）

- **三层**：核心（启动自动写、不可反初始化）：`send-to-agent`、`render-route`、`tw-web-host`；**起步**（首次安装默认写、可反初始化，`startup: true`）：`doc-note`、`starter-docs`，外加 **gated 三项 `publish-spec`（v0.23.0）/ `wechat-setup`（v0.23.1）/ `wechat-publish`（v0.23.3，工具栏按钮插件）**（只在 `wechat.enabled` 开启时写，见下条 gate）；可选（默认不写、可反初始化）：`home-index`、`all-articles`、`ui-styles`、`menubar-theme`、`clip-bridge`（剪藏桥使用说明文档 seed，ONE-SHOT + marker；真功能在 `clip-bridge.ts`）。
...
- **`gate`（v0.23.0，可选功能的启动闸）**：`SeedDef.gate?: (ctx: SeedContext) => boolean`，只在 **`runAllSeeds`（启动路径）** 生效——返回 false 就整个跳过该 seed。用于**属于可选功能**的内容（`publish-spec` ← `ctx.wechat`，来自 config `wechat.enabled` 默认 false）：不开该功能的用户 wiki 里不该出现它的文档。⚠️ 设置页手动「初始化/重新初始化」走 `runSeedById`，**不过 gate**（那是显式请求）；`checkAllSeeds` 也不受影响（仍如实报告该文档「缺失」）。新增可选功能 seed 时照此模式加 `gate`，并在 `verify-seed-error-policy.mjs` 补「默认跳过 / 开启后参与」两条断言。
...
- **微信后台自动化：轮询里绝不能读 `document.body.innerText`（v0.23.0 实测）**：微信编辑器 DOM 极大，读一次 `body.innerText` 强制整页 layout，**单次 evaluate ≈17 秒**；早期 `tools/wechat/weixin-flow.js` 的 `saveDraft`/`publishDraft` 用它找成功提示，8 次轮询把命令拖过 210s 超时——**而草稿其实已保存成功**（假阴性，最坑的一种）。改成只查少量 toast 节点 + URL 后，`saveDraft` 从 **137 秒降到 3.9 秒**。`scripts/verify-wechat-adapters.mjs` 有源码级回归守门（已反向验证）。同类教训：**任何在大 DOM 上的轮询都不要读全页 innerText**。
- **写 tiddler 必须「先读全文再整体写回」（v0.23.0 血泪）**：`tiddlywiki_batch_put` 要求每条带 `text`（源码里 `typeof item.text !== 'string'` 直接抛「缺少 text」），裸 REST PUT 一段文字会把笔记正文**整个覆盖成空**。真实踩过：给「自救计划 1」标记发布状态时只 PUT 了一小段文字，正文被清空，靠 `git checkout` + 重启 TW 才恢复。**正确姿势**：先 GET 完整 tiddler（含 text/tags/type/created），只改要改的字段，再整体写回——`tools/wechat/backfill-publish-state.mjs` 就是这个模式，且**默认 dry-run**。
...
- **重启 TW 之前必须把 syncer 队列排干（v0.18.0 → v0.22.0 的四次教训，合并成一条）**：`waitForFileWrite` 必须传 `newerThanMs`（等 **mtime 前进**），只判「存在」会在文件早就存在时立刻返回；等一个文件不够，要再跑 `flushPendingWrites()`；而**单枚哨兵也证明不了队列排干**——TW syncer 会跳过最近 1s 内保存过的标题（`syncer.js` 的 `throttleInterval` / `chooseNextTask`），哨兵是另一个标题、随时可写，会「插队」先落盘，被 throttle 的写入随即被重启吞掉（v0.22.0 新增 marker 写入后 `tw-web-host` **每次**都这样丢，verify-seeds-admin 稳定复现）。另外两个静默陷阱：哨兵按内容类型落成 `.txt`/`.json`/`.tid`，别写死扩展名判定；Windows 上 mtime 可能同刻。现在 `flushPendingWrites` 是**两段式**：写哨兵 A 等落盘 → 睡一个 throttle 窗口（读 `$:/config/SyncThrottleInterval`，默认 1s）→ 再写哨兵 B 等落盘。**所有重启路径**（seed 重启 / 启动自举 / `/sync` 与工具 `git_sync` / 知识库切换）都必须先走过它。
- **匿名 spawn 分支必须带 `anon-username=GUEST`（v0.22.9 教训，用户实测报障）**：TW 判定「是否已登录」用的是 `isLoggedIn = json.username !== "GUEST"`（`tiddlywebadaptor.js:93`）。不传 `anon-username` 时 `/status` 回 `""`，`"" !== "GUEST"` 为真 → 匿名读者被当成已登录 → `syncer.js:281-283` **每次页面加载**都把 `$:/status/UserName` 覆写成空串，用户填的「编辑者署名」静默消失；若 `$:/config/SyncFilter` 放行了 `UserName`，空串还会落盘进 git。**`GUEST` 是承重哨兵**：任何人名（含用户真名）都仍算已登录。⚠️ 别被那句已删掉的旧注释误导——**只有 `readers`/`writers` 会 401**（显式 `readers` 让 `(anon)` 不再是 principal），`anon-username` 单独传完全安全。守门 `scripts/verify-anon-username.mjs`（进 `verify:e2e`），含反向验证。
- **「就绪」不能用单一硬超时判定（v0.22.5 教训，用户实测报障）**：一个 5099 条目 / 26MB 的知识库在**冷文件缓存**下，从 `spawn` 到 TW 打印 `Serving on` 实测要 **~44s**（证据在 `/status` 环形日志：`01:43:51 spawn` → `01:44:11 did not become ready in time` → `01:44:35 [out] Serving on`）。旧代码只有一个 20s 硬超时，于是两件事同时发生：① 抛错让 `startupTask` 的 catch 吞掉后续——**配置 tiddler 没加载、核心 seed 没写、剪藏桥没起**（都会拖到下次重启）；② `waitReady` 写了 `health='failed'` + 粘性 `error`，而**超时后没有任何东西再探测**，TW 明明在服务、面板/FAB 却一直报故障。所以判定必须是「软窗口只警告 → 硬上限才失败 → 失败后仍复探」，且**绝不在超时路径上杀子进程**（重启一个正在慢慢载入的 wiki 只会更慢）。改这块前先读 `src/host/ready-policy.ts` 的模块头，并跑 `scripts/verify-ready-policy.mjs`（含「44s 冷启动必须判成功」这条事故形状的回归断言、以及「`wiki.ts` 不得再出现 20s 硬编码」的源码级断言）。
...
- **复用既有草稿 ≠ 可以覆盖它（v0.22.8 教训，用户数据丢失）**：`/edit` 的 v0.20.0 修复只解决了「给已存在的草稿另起一个时间戳标题」，但**仍然把 `draftText` PUT 进刚复用的那个草稿**——而那个草稿里装的是用户在 TW 原生编辑器里敲进去、**尚未保存**的内容。触发路径不需要任何特殊操作：打开快速笔记（标题是分钟级时间戳）→ 写点东西 → 关掉弹窗 → **同一分钟内**再打开（标题相同、调用方文本为空）→ 草稿被笔记已保存的正文覆盖。规则：**我们创建的草稿才由我们填充；复用的草稿只有调用方显式给了文本才写**（`draftExists` 标记）。`verify-audit-fixes.mjs` 有回归（还原修复即红）。
...
- **iframe 没赋过 `src` 时，`iframe.src` 是宿主页面自己的 URL（v0.22.3 教训）**：`frame.src = frame.src` / `''.split('#')[0] + '#标题'` 这两类写法在「iframe 还没载入、但已经 `hidden = false`」（`setVisible(true)` 先于首个 `/status` 响应）时会把 **DSH 页面**载进 iframe——iframe 里再起一份 DSH，重复 FAB / 重复全局监听 / 白屏。判据只能是 `iframe.dataset.loaded`（只有 `showFrame` 写它），统一走内核的 `loadableFrameUrl()`。守门：`verify-frame-guards.mjs`（结构断言）+ `verify-frame-surface.mjs`（DOM 级行为断言），都在 `verify:unit`。
- **TW frame 生命周期只能有一份实现（v0.22.4 教训）**：上一条 bug 的根因不是「谁写错了」，而是 `panel.ts` 与 `tw-frame.ts` **各抄了一份** showError/showStarting/showFrame/fallbackLoad/doRefresh——修的时候守卫只补到 panel，tw-frame 那份照旧，于是同一个坑踩了两次。现在这些函数只在 `tw-frame.ts` 的 `createTwFrameSurface(skin)` 里各有一份，surface 只提供**皮肤（类名）+ 惰性构建点 + 可见性**。往 panel 里加 frame 相关逻辑前先想一遍：是不是该加在内核里？`verify-frame-guards.mjs` 会用计数断言把「第二份」直接判红。
  - 三条顺序/语义约定，改的时候别踩：① panel 打开链接必须**先 `state.openPanel()` 再 `surface.openTiddler()`**（内核的 `visible` 由 `setVisible` 驱动，顺序反了内核以 `visible=false` 拒绝，链接静默丢失）；② 内核只在**已载入 TW 地址**时才显示 frame，`setVisible(false)` 会隐藏 frame 并停掉有界轮询（下次显示重新给满预算）；③ **FAB「重载面板」不受可见性限制**——`onReloadRequest` 的唯一判据是 `dataset.loaded`，刻意不加 `frame.hidden`：显式重载要覆盖**关掉的**面板，否则「关掉面板 → 点重载 → 再打开」会看到旧资源（v0.22.4 取舍；`verify-frame-guards.mjs` 会拦住重新加回这个判据，`verify-frame-surface.mjs` 也断言隐藏 surface 会被重载）。
- **快速笔记原生编辑弹窗的 iframe 会「空掉」（v0.22.6 教训，用户实测报障）**：`openEditorPopup()` 只在 **URL 变化**时给 iframe 赋 `src`（赋值即整页重载，会丢 TW 里未保存的草稿/滚动/撤销，所以刻意如此）。但 TW 的编辑器「删除」是 `tm-delete-tiddler`：它把原 tiddler 与草稿一起删掉、`removeTitleFromStory` 把 story 清空——而**主机重建的草稿标题往往与删除前完全相同**（时间戳精确到分钟 + canonical `Draft of "…"` 复用），于是 URL 不变、不重载，弹窗里只剩白板，再点「快速笔记」也只会把这块白板关了又开。现在三处一起兜住：① `isEditorPopupBlank()`（同源读 iframe 的 `.tc-story-river`，**一条 `.tc-tiddler-frame` 都没有**即判空；跨源/仍在 loading/读不到文档一律**不**判空，绝不误伤正在编辑的编辑器）让 `openEditorPopup()` 强制重载；② `closeEditorPopup()` 清掉 `frame.dataset.loaded`——关闭再打开必须是「新的编辑器载入」，而不是把上一次的 view 状态原样摆回来；③ dock 与 `openNative()` 的「已打开」幂等守卫改成 `isEditorPopupOpen() && !isEditorPopupBlank()`。守门：`scripts/verify-editor-popup.mjs`（进 `verify:unit`，11 条行为断言，反向验证过）+ `verify-frame-guards.mjs` 的接线断言。同类坑：**card 模式底部操作条不要给按钮留收缩余地**——340px 卡片里 5 个控件本就放不下，flex 默认收缩会把按钮文字压成两行（实测每个按钮 `textRects=2`、高 46px）；按钮必须 `white-space:nowrap` + `flex:0 0 auto`，放不下时让**整组换行**（右组 `margin-left:auto` 仍贴右）。
...
- **首页「快速记笔记」的日期模板只能用 TW 的日期 token（v0.22.1 教训）**：`formatDateString` 的 token 是 `YYYY`/`0MM`/`0DD`/`0hh`/`0mm` —— **没有 `HH`**，未知字符被**原样输出**，于是 `<now "YYYY-MM-DD-HHmm">` 生成 `2026-9-11-HH20` 这种标题（用户实测报障）。要补零一律用 `0` 前缀（`0MM`/`0DD`/`0hh`/`0mm`）。守门在 `verify-constants.mjs`（首页 seed 不得含 `HHmm`）。同一处还有一个 TW 命名空间约定：`$:/state/*` 会被 syncer **落盘进 git**，浏览器端的临时反馈应写 **`$:/temp/*`**（tiddlyweb adaptor 显式排除 `$:/temp`）。
...
- **`$:/temp` 条目在 iframe 的 story view 里永远无法正常显示（v0.16.11–19）**：recipe 列表与 tiddlyweb adaptor 都排除 `$:/temp`，且 TW 核心的 `$:/config/ViewTemplateBodyFilters/system` 会把 `$:/temp/` 一律按**代码块**渲染。**所以汇总 Tab 一律走 `/tw/render` 片段管线，不要用 iframe / story view**；服务端直连 REST（tw-api）不受影响。
...
- **别向 dsh-better-sidebar 注册 tab kind**：它的 id 是**全局 tab kind**，重复注册即报 `tab kind "dsh-tiddlywiki" is already registered`。v0.17.0 起插件完全不引用 `ctx.betterSidebar`；要重新支持必须用私有前缀（如 `dsh-tiddlywiki/tw`）并做幂等注册。
- 消息里**附加说明放消息末尾**（正文之后），别插在待办说明与正文之间。
- **TW 服务端写路径不补 `created`/`modified`（v0.22.10 教训）**：`put-tiddler.js` 只做 `state.wiki.addTiddler(new $tw.Tiddler(fields,{title}))`——`$tw.wiki.getCreationFields()`/`getModificationFields()` **只在 TW 自己的浏览器 UI 里**被调用，任何「服务端会补时间戳」的假设都是错的（插件曾据此把这两个字段放进 `CLEAN_SKIP_FIELDS`，结果落盘只有 tags/title/type）。后果不是「少个字段」这么轻：`sortTiddlers` 取排序字段是 `tiddler.fields[sortField] || ""`，缺 `modified` 的条目在 **`+[!sort[modified]]` 降序时直接沉到最后一名**（用户现象：新日记在「主题页·日志」排 175/175，看着像没被收录）。凡新增写路径：优先走 `buildWriteTiddler()`（它已负责时间戳），手拼 PUT body 时记住 `TiddlyWebClient.put()` 的 `ensureTiddlerTimestamps()` 只会**补缺**、不会替你决定语义（保留 `created` 还是重置）。存量缺字段的条目不会被自动回填——用 wiki 根目录的 `backfill-timestamps.mjs`（默认 dry-run，按 git 历史取时间，`--write` 才落盘）。
- **`sort[modified]` 类页面「少了一条」先查字段而不是过滤器**：TW 的排序对缺字段条目不是「排在后面一点」，而是**排到最后一名**；用户报「新笔记没出现在汇总页」时，先 `tiddlywiki_get` 看 `modified` 在不在，再怀疑 filter。

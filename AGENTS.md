...
| 项 | 当前值 | 位置 |
|---|---|---|
| **插件版本** | `0.16.23`（npm latest = 0.16.23 发布后；git tag `v0.16.23`） | `package.json` `version` |
...
│   ├── better-sidebar-tab.ts # DSH Better Sidebar TW tab（v0.16.23）：ctx.betterSidebar.registerTab 注册 + 互斥（body 复用 tw-frame）

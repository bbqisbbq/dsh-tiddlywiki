...
tools/wechat/           # **发布到微信公众号**（v0.23.0，可选能力，不进 host 运行时）：wechat-html.js（语义 HTML → 内联样式装饰器，主题表在此）/ weixin-flow.js（浏览器流程：填表/insertHTML/传图/封面/存草稿/发表）/ create-article.js（本地 HTML → 草稿）/ publish-note.js（**主入口**：TW 笔记 → 草稿，含 --preview）/ install-wechat-adapters.mjs（一键装到 ~/.opencli/clis/weixin/ 并自检）。⚠️ 这三个 .js 是**用户态的 opencli adapter**，不进 lib/、不被 typecheck 覆盖

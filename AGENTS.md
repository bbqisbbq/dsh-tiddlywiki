...
| **Seed 注册表（12 项，三层）** | 核心（自动写、不可移除）：`send-to-agent`、`render-route`、`tw-web-host`；起步（首次安装默认写、可移除）：`doc-note`、`starter-docs`、**gated 两项（v0.23.0/0.23.1，`startup: true` + `gate: (ctx) => ctx.wechat === true`——**只在设置页开启「微信发布」(`wechat.enabled`) 时才写**，`gate` 只作用于启动路径、设置页手动「初始化」不受它约束）：`publish-spec`（v0.23.0「发布元数据规范」——开启时提示词里有 `[[发布元数据规范]]` 指针，文档不落地就是死链）与 `wechat-setup`（v0.23.1「微信公众号发布指南」——**逐字节等于 `docs/wechat-publish-setup.md`**，由 `scripts/gen-seed-wechat-docs.mjs` 生成，`scripts/verify-wechat-docs-seed.mjs` 守逐字节一致；两篇同进同退）**；可选（手动）：`home-index`、`all-articles`、`ui-styles`、`menubar-theme`、`clip-bridge`（剪藏桥使用说明，真功能在 `clip-bridge.ts` 运行时代码里）。文档类内容统一打 `dsh-docs` 标签（进首页「📚 插件文档」栏）。**v0.22.0 起**：有 `markerTitle` + `content` 的 seed 会在标记里记**内容哈希**（`{version,hashes,at}`）以支持「有更新/本地已修改」检测；`doc-note` 的正文**由工具注册表生成**（`docNoteText(tools)`，不再手抄工具清单） | `src/host/seeds.ts` 的 `SEED_DEFS` + `src/host/seed-util.ts`（`hashText`/`parseSeedMarker`/`readSeedMarker`/`writeSeedMarker`） |
...
| **配置项** | `wikiRoot`/`wiki`（**v0.22.0 起只是默认值**：运行中实际位置 = 指针文件 > cordis config > 内置默认）/`port`/`git{autoCommit,debounceMs,remote,branch}`/`note{tag}`/`startup{readyTimeoutMs}`（**v0.22.5**：软就绪窗口，默认 60000、夹 5s–600s，硬上限 = 3×，对下次启动/重启生效）/`prompt{enabled,mode,extra,override}`/`bridge{enabled,port,token,tag}`/**`wechat{enabled}`（v0.23.0：可选功能开关，默认 `false`——关时不注入发布提示词、不写「发布元数据规范」与「微信公众号发布指南」两个 gated seed；开时仅多这提示词一行 + 两篇文档，不安装任何东西）**/`ui{showQuickNote,showQuickNoteDock,quickNoteMode,sidebarLabel,showPanelStatus,showSyncButton,followDshTheme,darkPalette,sendToAgent{enabled,endpoint,token},allArticles{pageSize},tabLabel,showSessionTab,showRightbarTab}`/`uiLanguage`/`auth{username,password}` | `src/host/config.ts` |
| **DSH 路由** | `/status` `/note` `/edit` `/tags` `/recent` `/get` `/search` `/render` `/sync` `/upload` `/restart` `/session/summary` `/agent/sessions` `/agent/modes` `/agent/send` `/agent/create` `/api/*` `/tw/*`；admin：`/admin/state` `/admin/prompt`（**v0.22.7**：GET 已保存／POST 草稿预览） `/admin/info` `/admin/config` `/admin/restart` `/admin/seeds` `/admin/seeds/run` `/admin/seeds/remove`／**v0.22.0 知识库位置**：`/admin/wiki/location`（GET 读）、`/admin/wiki/switch`（POST 切）、`/admin/wiki/reset`（POST 恢复默认） | `src/host/routes.ts` + `src/host/admin.ts` |
...
- **三层**：核心（启动自动写、不可反初始化）：`send-to-agent`、`render-route`、`tw-web-host`；**起步**（首次安装默认写、可反初始化，`startup: true`）：`doc-note`、`starter-docs`，外加 **gated 两项 `publish-spec`（v0.23.0）/ `wechat-setup`（v0.23.1）**（只在 `wechat.enabled` 开启时写，见下条 gate）；可选（默认不写、可反初始化）：`home-index`、`all-articles`、`ui-styles`、`menubar-theme`、`clip-bridge`（剪藏桥使用说明文档 seed，ONE-SHOT + marker；真功能在 `clip-bridge.ts`）。

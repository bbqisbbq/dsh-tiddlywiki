...
npm run verify:unit   # 剪藏桥（含 IPv6/保留网段 SSRF 回归）/ seed 读失败策略 / 渲染片段净化器 / 配置密钥遮掩（含 auth.password，v0.20.0）/ **写策略纯函数（type 保留，v0.20.1）** / **注入提示词（slim 无参数清单、full 与工具注册表逐项一致、治理约定不丢、**草稿预览与保存路径逐字节一致，v0.22.7**，v0.21.0）** / **客户端 frame 守门（v0.22.3：空 src 陷阱、活动主题优先级、跨源 try/catch、共享 toast、死标记；v0.22.4 增「生命周期只有一份」去重断言）** / **客户端 frame 内核行为单测（v0.22.4，tsx 直跑源码 + DOM 打桩：空 src 两条重载路径、可见性不显示空 frame、URL 不重复赋值、启动/错误/重试恢复、hash 导航与兜底、dispose 幂等）** / **启动就绪策略（v0.22.5，tsx + 虚拟时钟：44s 冷启动必须判成功、慢启动只警告一次、子进程退出即 exited、硬上限才 timeout、轮询节奏有界、wiki.ts 不得再有 20s 硬编码）** / **快速笔记原生编辑弹窗（v0.22.6，tsx + DOM 打桩：同 URL 有内容不重载、story 被删空必须重载、关闭清 `dataset.loaded`、关闭后重开必重载、空白判据的四种保守边界）** / 客户端 status-cache 合并（tsx 直跑源码）
npm run verify:e2e    # auth 模式 / **verify-anon-username（v0.22.9：真起 TW，`/status` 必须回 GUEST 哨兵、落盘署名不被覆盖、匿名仍可写、auth 模式不受影响且匿名仍 401）** / 工具层 / **审计回归 verify-audit-fixes**（attach 覆盖保护 / 草稿类型 / 回收站索引 / delete 并发 / rename 部分失败 / 汇总未探测 / batch_put 并发顺序）/ git 冲突解决 / 崩溃自愈+并发写 / seed 两个 E2E / **verify-wiki-switch（v0.22.0：真起两个 wiki，切换 / 回滚 / 指针文件 / 非法输入）**

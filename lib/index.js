...
async function openInTwEditor(client, title, text, tags, options = {}) {
	...
	let canonicalProbe;
	try {
		canonicalProbe = await client.get(canonical) === void 0;
	} catch {
		canonicalProbe = void 0;
	}
	if (canonicalProbe === false) draftTitle = canonical;
		...
		if (draftTitle === void 0) draftTitle = canonical;
		else if (draftTitle !== canonical) draftTitle = `${canonical} ${Date.now()}`;
	} else draftTitle = `${canonical} ${Date.now()}`;
	await client.put({
		title: draftTitle,
		text: draftText,
		"draft.of": title,

...
var GitFace = class {
	...
	*  Serialized against `commit()` (index.lock). */
	...
	async pullUnlocked(dir) {
		...
		return {
			ok: false,
			message: files.length > 0 ? `conflict in ${files.join(", ")} (rebase aborted)${note}: ${reason}` : `pull failed${note}: ${reason}`,

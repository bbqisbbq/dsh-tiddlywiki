...
var TiddlyWebClient = class TiddlyWebClient {
	...
	*/
	async listTags(limit) {
		...
		if (limit === void 0) return tags;
		return tags.slice(0, Math.max(1, Math.min(Math.floor(limit), 1e3)));
	}
...
var WikiServer = class {
	...
	*/
	...
	async waitReady() {
		...
		const headers = this.options.username !== void 0 && this.options.username.length > 0 ? { authorization: `Basic ${Buffer.from(`${this.options.username}:${this.options.password ?? ""}`, "utf8").toString("base64")}` } : void 0;
		for (;;) {
			if (this.child === void 0) throw new Error("wiki process exited before ready");
			try {
				if ((await fetch(`${this.url}/status`, {
...
function buildWikitext(sessionId, collected, producedTitles, readTitles, stateByTitle) {
	...
	const entryLine = (title, state, entry) => {
		const bits = [`[[${title}]]`];
		if (state !== void 0 && state.unknown === true) bits.push("⚠️ 状态未知（查询失败）");
		else if (state === void 0 || !state.exists) bits.push("⚠️ 已删除/不存在");
		else {
...
function decodeEntities(value) {
	return value.replace(/&#x([0-9a-f]+);?/gi, (_m, hex) => {
...
*/
async function writeSeedMarker(client, title) {
	try {
		await client.put({
			title,
			text: "seeded-once",
...
var ConfigStore = class {
	...
	*/
	async load(client) {
		...
		try {
			const parsed = JSON.parse(typeof tiddler.text === "string" ? tiddler.text : "");
			this.overrides = isPlainObject(parsed) ? parsed : {};
		} catch {
...
async function flushPendingWrites(client, tiddlersDir, timeoutMs = 8e3) {
...
function assertNoConflict(title, existing, expected) {
	...
	if (!(expected.expectedModified !== void 0 || expected.expectedRevision !== void 0)) return;
	const expectedMs = parseTiddlerDate(expected.expectedModified);
	const currentMs = parseTiddlerDate(existing.modified);
	if (expectedMs !== void 0 && currentMs !== void 0 && expectedMs === currentMs) return;
	const currentRevision = existing.revision;
	if (expected.expectedRevision !== void 0 && currentRevision !== void 0 && String(currentRevision) === String(expected.expectedRevision)) return;
	throw new WriteConflictError(`写入冲突：tiddler「${title}」在你读取之后已被改动（当前 revision=${currentRevision ?? "?"} modified=${toIsoDateString(existing.modified) ?? "?"}；期望 revision=${expected.expectedRevision ?? "?"} modified=${expected.expectedModified ?? "?"}）。请重新读取最新内容后重试；确认要用你的版本覆盖时可传 force: true。`);
}
//#endregion
...
async function openInTwEditor(client, title, text, tags, options = {}) {
	...
	if (draftTitle === void 0) {
		const canonical = `Draft of "${title}"`;
		let free = false;
		try {
			free = await client.get(canonical) === void 0;
		} catch {
			free = false;
		}
...
function registerRoutes(ctx, deps) {
	...
	*  order, plus `total`/`truncated` so a capped caller can say "N of M". */
	...
	const handleRecent = async (req, res) => {
		...
		try {
			const limit = readLimit(new URL(req.url ?? "/", "http://127.0.0.1"), 15);
			json(res, {
				ok: true,
				limit,
	...
	const handleSync = async (req, res) => {
		...
		try {
			...
			if (pulled.changed === true) try {
				...
				if (flushClient !== void 0) await flushPendingWrites(flushClient, join(dir, "tiddlers")).catch(() => void 0);
				await deps.server.restart();
				restarted = true;
			} catch (err) {
	...
	*/
	...
	const isBlockedProxyPath = (pathname) => {
		const at = pathname.indexOf("/tiddlers/");
		if (at < 0) return false;
		const raw = pathname.slice(at + 10);
		let title = raw;
		try {
			title = decodeURIComponent(raw);
		} catch {}
			...
			if (Array.isArray(body.languages)) {
				...
				if (client !== void 0) {
					...
					await client.put({
						...
						console.warn("[dsh-tiddlywiki] pinning $:/language failed:", err);
					});
	...
	const handleSeedsRun = async (req, res) => {
		try {
			...
			if (needsRestartAfterSeeds(results)) try {
				...
				if (!await flushPendingWrites(client, join(deps.getWikiPath(), "tiddlers"))) console.warn("[dsh-tiddlywiki] seed writes may not have been flushed before restart");
				await deps.server.restart();
				restarted = true;
			} catch (err) {
				restartError = err instanceof Error ? err.message : String(err);
...
async function readTrashIndex(wiki) {
	...
	if (tiddler === void 0 || typeof tiddler.text !== "string") return [];
	try {
		const parsed = JSON.parse(tiddler.text);
	...
	*  wiki-facing tools (git tools operate on the repo path instead). */
	...
	register(defineTool({
		...
		parameters: {
			query: {
				type: "string",
				description: "搜索关键词（大小写不敏感，子串匹配；命中标题/标签/正文，标题命中权重最高）",
				required: true
			},
			tags: {
				type: "array",
		...
		output: {
			...
			render: (_args, value) => {
				...
				const lines = [`TiddlyWiki 搜索「${value.query}」${filters.length > 0 ? ` (${filters.join(" · ")})` : ""}：命中 ${value.total} 条（按相关度排序）。`];
	...
	register(defineTool({
		...
		output: {
			...
			render: (_args, value) => {
				...
				for (const r of value.items) lines.push(`- ${r.title}：${r.written ? "已写入" : r.skipped ? "已跳过（存在）" : `失败（${r.error ?? "未知错误"}）`}`);
				return [{
					type: "text",
					text: lines.join("\n")
				}];
	...
	register(defineTool({
		...
		execute: async (args) => {
			const wiki = requireWiki();
			const existing = await wiki.get(args.title);
			if (!(existing !== void 0 && args.permanent !== true && !args.title.startsWith("$:/dsh-tiddlywiki/trash/"))) {
	...
	register(defineTool({
		...
		parameters: {
			...
			text: {
				type: "string",
				description: "要追加/前插的文本（Markdown）",
				required: true
			},
			mode: {
				type: "string",
		...
		execute: async (args) => {
			...
			if (existing !== void 0) await wiki.put({
				...cleanTiddler(existing),
				text: next
			});
			else {
				const tags = finalTagsForWrite(title, void 0, Array.isArray(args.tags) ? args.tags.filter((t) => typeof t === "string" && t.trim().length > 0) : [], true);
				const tiddler = {
					title,
					text: next
				};
	...
	register(defineTool({
		...
		parameters: {
			...
			checks: {
				type: "array",
				items: { type: "string" },
				description: "可选：只跑指定检查（junk-tags / broken-links / empty-notes / missing-type）"
			}
		},
		output: {
			schema: { type: "json" },
...
//#endregion
export { AGENT_WRITTEN_TAG, ALL_ARTICLES_MARKER_TITLE, ALL_ARTICLES_TEXT, ALL_ARTICLES_TITLE, AutoCommitter, CLIP_BRIDGE_BOOKMARKLET, CLIP_BRIDGE_DEFAULT_PORT, CLIP_BRIDGE_DOC_TEXT, CLIP_BRIDGE_DOC_TITLE, CLIP_BRIDGE_DRAG_HREF, CLIP_BRIDGE_MARKER_TITLE, ClipBridge, ConfigStore, DEFAULT_NOTE_TYPE, DOC_NOTE_TAG, DOC_NOTE_TEXT, DOC_NOTE_TITLE, DSH_DOCS_TAG, GitFace, HOME_DEFAULT_TIDDLERS, HOME_INDEX_ITEMS, HOME_INDEX_MARKER_TITLE, HUMAN_EDITED_TAG, MARKDOWN_PLUGIN, MASKED_SECRET, MENUBAR_THEME_MARKER_TITLE, MENUBAR_THEME_TEXT, MENUBAR_THEME_TIDDLER, MISSING_TYPE_FILTER, PATH_PREFIX, RENDER_BUNDLE_TEXT, RENDER_MARKER_TITLE, RENDER_PLUGIN_FILE, RENDER_PLUGIN_TITLE, SEED_DEFS, SEND_TO_AGENT_BUNDLE_TEXT, SEND_TO_AGENT_MARKER_TITLE, SEND_TO_AGENT_PLUGIN_TITLE, SESSION_SUMMARY_PREFIX, STARTER_DOCS_ITEMS, STARTER_DOCS_MARKER_TITLE, TEXT_LIST_FILTER, TW_PROXY_PATH, TW_PROXY_PREFIX, TW_WEB_HOST_DEFAULT, TW_WEB_HOST_TIDDLER, TiddlyWebClient, UI_STYLES_MARKER_TITLE, UI_STYLE_ITEMS, WikiServer, WriteConflictError, apply, assertNoConflict, assertPublicImageUrl, buildBinaryTiddler, buildClipTiddler, buildImageNoteTiddler, buildWriteTiddler, bundledCatalog, checkAllSeeds, cleanTiddler, deepMerge, defineTool, downloadClipImage, dshHomePath, ensureLanguage, ensurePlugin, ensureTwWebHost, escapeInline, flattenTiddlerFields, flushPendingWrites, hostAllowed, imageExtensionForMime, inject, isBinaryType, isPrivateAddress, isSafeUrl, maskConfigSecrets, name, needsRestartAfterSeeds, normalizeThemes, openInTwEditor, parseClipPayload, pickImageMime, readWikiInfo, registerAdminRoutes, registerRoutes, registerTiddlywikiTools, removeSeedById, resolveClipTitle, resolveTwRoot, runAllSeeds, runSeedById, sanitizeTwFragment, seedAllArticles, seedClipBridge, seedDocNote, seedHomeIndex, seedMenubarTheme, seedRenderRoute, seedSendToAgent, seedStarterDocs, seedUiStyles, stripMaskedSecrets, unseedClipBridge, waitForFileWrite, writeSessionSummary, writeWikiInfo };

...
var GitFace = class {
	...
	*/
	async conflictState(dir, options = {}) {
		...
		for (const file of changed.slice(0, MAX_CONTENT_SCAN_FILES)) if (this.fileHasConflictMarkers(join(dir, file))) marked.push(file);
...
* 当成收尾符削掉。
*/
function trimTrailingClosers(raw) {
	let s = raw;
	for (;;) {
		...
		if (type === "tool/call") {
			...
			switch (name) {
				...
				case "tiddlywiki_delete":
					if (typeof args.title === "string" && args.title.length > 0) record(collected.deleted, {
						title: args.title,
						action: "deleted",
						time: t,
						subagent
				...
				case "tiddlywiki_trash": {
					...
					const trashTitle = typeof args.title === "string" && args.title.length > 0 ? args.title : void 0;
					if (trashTitle === void 0) break;
					if (trashAction === "restore") record(collected.produced, {
						title: trashTitle,
						action: "produced",
						time: t,
						detail: "回收站恢复",
...
function escapeInline(text, max = 200) {
	return text.slice(0, max).replace(/[\u0000-\u001f\u007f]/g, " ").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/[[\]|{}]/g, " ");
}
...
function buildWikitext(sessionId, readFailed, collected, producedTitles, readTitles, deletedTitles, stateByTitle) {
	const lines = [];
	lines.push("! 会话相关 wiki 汇总");
	lines.push("");
	lines.push(`本页列出会话 \`${escapeInline(sessionId, 120)}\`（含其后代子代理）在本会话中产生 / 读取 / 检索 / 删除过的知识库笔记。`);
	lines.push("");
	...
	const anySubagent = [...collected.produced.values()].some((e) => e.subagent) || [...collected.read.values()].some((e) => e.subagent) || [...collected.deleted.values()].some((e) => e.subagent) || collected.searches.some((s) => s.subagent);
	...
	lines.push("| 产生 📝 | 读取 👀 | 检索 🔍 | 删除 🗑 | 涉及会话 |", "| --- | --- | --- | --- | --- |", `| ${producedCount} | ${readCount} | ${searchCount} | ${deletedCount} | ${anySubagent ? "本会话 + 子代理" : "本会话"} |`);
	lines.push("");
	const entryLine = (title, state, entry) => {
		const bits = [`[[${title}]]`];
		if (state === void 0) bits.push("（本轮查询预算用尽，未探测）");
...
async function writeSessionSummary(client, sq, sessionId) {
	...
	const text = buildWikitext(sessionId, readFailed, collected, producedTitles, readTitles, deletedTitles, await enrichTitles(client, [.../* @__PURE__ */ new Set([
		...producedTitles,
		...readTitles,
		...deletedTitles
	])].slice(0, MAX_ENRICH_TITLES)));
...
- 打开任意笔记，点正文底部的**标签丸**，或侧边栏「标签」（标签云）里点某个标签；
- 内核会弹出一个**下拉列表**，列出所有带此标签的笔记（还能拖拽排序、直接跳转）。
- 优点：一行代码都不用写；缺点：列表**全量不分页**（标签下笔记很多时会很长）、不能加说明文字、不能自定义排序/样式，也做不了「多主题合一」。
- ⚠️ 注意：它只是个弹层，**不是**一个可以打开、可以分享链接的「标签页面」——直接打开以标签命名的条目，看到的只是那个条目本身（不存在则是缺失提示），**不会**自动列出成员笔记。想要「一个页面 + 可翻页」的标签列表，用方法二自己写汇总页（笔记数量多时给它套上分页）。
...
function deepMerge(base, over) {
	...
	for (const [key, value] of Object.entries(over)) {
		...
		if (isPlainObject$1(value) && isPlainObject$1(out[key])) out[key] = deepMerge(out[key], value);
		else out[key] = value;
	}
	return out;
}
...
const NUMBER_CONFIG_RANGES = {
	"bridge.port": [1, 65535],
	"git.debounceMs": [0, 36e5],
	"startup.readyTimeoutMs": [5e3, 6e5],
...
*/
const BOOLEAN_CONFIG_PATHS = /* @__PURE__ */ new Set([
	"bridge.enabled",
	"git.autoCommit",
	"note.workspaceMark",
	"prompt.enabled",
	...
	"ui.showPanelStatus",
	"ui.showQuickNote",
	"ui.showQuickNoteDock",
	"ui.showRightbarTab",
	"ui.showSessionTab",
	"ui.showSyncButton",
	"ui.sendToAgent.enabled",
	"wechat.enabled"
]);
/** Known STRING paths (kept permissive about the VALUE; only the type is enforced). */
const STRING_CONFIG_PATHS = /* @__PURE__ */ new Set([
	"auth.password",
	"auth.username",
	"bridge.tag",
	"bridge.token",
	...
	"ui.darkPalette",
	"ui.quickNoteMode",
	"ui.sendToAgent.endpoint",
	"ui.sendToAgent.token",
	"ui.sidebarLabel",
	"ui.tabLabel",
	"uiLanguage",
	"wechat.adapter",
...
const FORBIDDEN_PATCH_KEYS = /* @__PURE__ */ new Set([
	"__proto__",
	"constructor",
	"prototype"
]);
...
function normalizeConfigPatch(input) {
	if (!isPlainObject$1(input)) throw new ConfigPatchError("配置补丁必须是一个 JSON 对象（例如 {\"note\":{\"tag\":\"inbox\"}}）");
	const out = {};
	const walk = (source, target, prefix) => {
		for (const [key, value] of Object.entries(source)) {
			...
			if (range !== void 0) {
				if (typeof value !== "number" || !Number.isFinite(value)) throw new ConfigPatchError(`配置项 ${path} 必须是数字（收到 ${typeof value}）`);
...
var ConfigStore = class {
	...
	*/
	async load(client) {
		...
		try {
			...
			if (!isPlainObject$1(parsed)) throw new Error("config tiddler is not a JSON object");
	...
	*/
	async set(client, patch) {
		...
		if (storedText !== void 0 && storedText === nextText) return this.get();
		await client.put({
...
*/
function normalizeFieldsArg(fields) {
	if (fields === void 0 || fields === null) return void 0;
	if (typeof fields === "string") {
		...
		throw new Error(`fields 必须是一个对象（如 {"review-after":"2026-12-22"}），收到无法解析成对象的字符串：${excerpt(fields)}`);
	}
	if (isPlainObject(fields)) return fields;
	throw new Error(`fields 必须是一个对象（如 {"review-after":"2026-12-22"}），收到 ${Array.isArray(fields) ? "数组" : typeof fields}`);
}
/**
...
function applyCustomFields(tiddler, fields) {
	...
	if (!isPlainObject(fields)) throw new Error(`内部错误：fields 必须是对象，收到 ${Array.isArray(fields) ? "数组" : typeof fields}`);
...
function defineTool(options) {
	...
	return {
		...
		async execute(args, exec) {
			const normalized = options.normalizeArgs !== void 0 ? options.normalizeArgs(args) : args;
			...
			return userExecute(normalized, exec);
		}
	};
}
//#endregion
...
function registerRoutes(ctx, deps) {
	...
	* deployments.
	*/
	const twProxyAbsoluteBase = (req) => absoluteHostBase(req.headers.host, TW_PROXY_PATH);
	...
	*/
	const handleSessionSummary = async (req, res) => {
		try {
			...
			let pendingSummary;
			if (cachedSummary !== void 0 && Date.now() - cachedSummary.at < SUMMARY_REUSE_MS) pendingSummary = cachedSummary.value;
			else {
				pendingSummary = writeSessionSummary(client, sq, session);
				summaryInFlight.set(session, {
					at: Date.now(),
			...
			json(res, {
				...
				twUrl: TW_PROXY_PATH,
				twUrlAbsolute: twProxyAbsoluteBase(req)
			});
		} catch (err) {
			json(res, {
				ok: false,
	...
	*/
	const handleSearch = async (req, res) => {
		...
		try {
			...
			const explicitScope = tags.length > 0 || tag !== void 0 || field !== void 0 || value !== void 0;
			const narrowTag = workspace.length > 0 && !explicitScope ? `ws/${workspace}` : "";
			const narrowed = narrowTag.length > 0 ? await client.search(query, {
				tags: [narrowTag],
				tag,
				since,
				type,
			...
			const { items, total } = usedWorkspace ? narrowed : await client.search(query, {
				tags,
				tag,
				since,
				type,
...
*/
async function scanWikiRuntimePlugins(wikiPath) {
	...
	for (const file of files) {
		...
		if (file.startsWith("$__config_Plugins_Disabled_") && file.endsWith(".tid")) {
			...
			const title = parseMetaFields(header).get("title");
			if (typeof title !== "string" || !title.startsWith("$:/config/Plugins/Disabled/")) continue;
			const pluginTitle = title.slice(27);
...
async function bundledCatalog(twRoot) {
	...
	const scan = async (sub) => {
		...
		for (const dir of dirs) {
			...
			const declaredTitle = typeof info.title === "string" && info.title.length > 0 ? info.title : void 0;
			out.push({
				name: `tiddlywiki/${dir}`,
...
function registerAdminRoutes(ctx, deps) {
	...
	const handleConfig = async (req, res) => {
		try {
			...
			await deps.config.set(client, stripMaskedSecrets(patch, deps.config.get()));
...
*/
function normalizeFieldsInArgs(args) {
	if (!("fields" in args)) return args;
	return {
		...args,
		fields: normalizeFieldsArg(args.fields)
	};
}
/** batch_put 专用：逐条目归一化 `fields`（非对象条目原样留给逐条校验报错）。 */
function normalizeBatchItemsFields(items) {
	if (!Array.isArray(items)) return items;
	return items.map((item) => item !== null && typeof item === "object" && !Array.isArray(item) ? normalizeFieldsInArgs(item) : item);
}
/**
...
function expiryOf(value) {
	...
	if (dateOnly !== null) return new Date(Number(dateOnly[1]), Number(dateOnly[2]) - 1, Number(dateOnly[3]), 23, 59, 59, 999).getTime();
...
function registerTiddlywikiTools(ctx, deps) {
	...
	*  wiki-facing tools (git tools operate on the repo path instead). */
	...
	register(defineTool({
		...
		parameters: {
			...
			tags: {
				type: "array",
				items: { type: "string" },
				description: "标签数组（可选）。不传 = 保留既有条目的原标签；传空数组 [] = 清空全部标签；有内容 = 整体替换"
			},
			fields: {
				type: "object",
	...
	register(defineTool({
				...
				items: {
					...
					properties: {
						...
						tags: {
							type: "array",
							items: { type: "string" },
							description: "标签数组（可选）。不传 = 保留原标签；传空数组 [] = 清空全部标签"
						},
						fields: {
							type: "object",
							additionalProperties: true,
						...
						expectedModified: {
							type: "string",
							description: "可选（v0.25.0）：该条目的乐观并发令牌，来自 tiddlywiki_get 的 modified。覆盖已存在条目时若已被改动，只让这一条失败并说明原因，其余条目照常"
						}
					}
				}
			},
		...
		normalizeArgs: (args) => ({
			...args,
			items: normalizeBatchItemsFields(args.items)
		}),
		output: { render: (_args, value) => {
	...
	register(defineTool({
		...
		parameters: {
			...
			expectedModified: {
				type: "string",
				description: "可选（v0.25.0）：旧标题的乐观并发令牌，传 tiddlywiki_get 读到的 modified；不匹配则拒绝重命名（避免把人类刚改过的正文写进新标题）"
			},
			expectedRevision: {
				type: "integer",
				description: "可选：乐观并发保护的另一种令牌——tiddlywiki_get 返回字段里的 revision"
			},
			force: {
				type: "boolean",
				description: "可选：true 时忽略 expectedModified/expectedRevision 强制重命名（默认 false）"
			}
		},
		output: { render: (_args, value) => {
		...
		execute: async (args) => {
			...
			if (refsTiddlers === 0 && refsSkipped === 0) warning = "未找到任何其他 tiddler 引用旧标题；如确实需要，可手动补充链接。";
			if (refsSkipped > 0) {
				const note = `有 ${refsSkipped} 个引用者因在遍历期间被改动而跳过（${skippedTitles.join("、")}），它们里面仍指向旧标题，请手动确认或重跑一次重命名。`;
				warning = warning === void 0 ? note : `${warning} ${note}`;
			}
	...
	register(defineTool({
		...
		parameters: {
			...
			heading: {
				type: "string",
				description: "可选：append 时改为写入指定标题的位置（Markdown # 或 wikitext ! 标题，按标题文本匹配、忽略标题级别）。**落点 = 该标题之后、下一个任意级别标题之前**：若该标题后紧跟子标题，内容插在两者之间（即章节开头），**不是整节末尾**。标题不存在时不报错，改为追加到文末，并在回执与返回的 headingMatched=false 里明确说明（别把「没命中」当「写进去了」）"
			},
			createIfMissing: {
				type: "boolean",
			...
			tags: {
				type: "array",
				items: { type: "string" },
				description: "可选：标签（不传则保留既有条目的原标签，传空数组 [] 表示清空全部标签；新建条目会额外自动补 agent-written）"
			},
			fields: {
				type: "object",
				additionalProperties: true,
			...
			expectedModified: {
				type: "string",
				description: "可选：乐观并发保护。传 tiddlywiki_get 读到的 modified；若条目在你读取之后被改动（人类在 TW 编辑器里改过）则拒绝写入——本工具是整篇回写，这一步能防止吞掉别人的修改"
			},
			expectedRevision: {
				type: "integer",
				description: "可选：乐观并发保护的另一种令牌——传 tiddlywiki_get 返回字段里的 revision（注意 revision 是 TW 的内存计数器，重启后会复位，长时间跨度请用 expectedModified）"
			},
			force: {
				type: "boolean",
				description: "可选：true 时忽略 expectedModified/expectedRevision 强制写入（默认 false）"
			}
		},
		normalizeArgs: normalizeFieldsInArgs,
		output: { render: (_args, value) => {
			const headingNote = value.heading === null ? "" : value.headingMatched === false ? ` · ⚠️ 未找到标题「${value.heading}」，已改为追加到文末` : ` · 段落「${value.heading}」`;
		...
		execute: async (args, exec) => {
			...
			return {
				...
				...headingMatched !== void 0 ? { headingMatched } : {},
	...
	register(defineTool({
		...
		execute: async (args) => {
			...
			if (typeof args.noteTitle === "string" && args.noteTitle.trim().length > 0) {
				...
				if (note !== void 0 && isBinaryType(typeof note.type === "string" ? note.type : void 0)) throw new Error(`tiddlywiki_attach: noteTitle「${embedInto}」是二进制附件（type=${String(note.type)}），把嵌入链接写进去会写坏附件；请改用普通笔记，或去掉 noteTitle 参数单独保存附件。`);
	...
	register(defineTool({
		...
		parameters: {
			...
			checks: {
				type: "array",
				items: { type: "string" },
				description: "可选：只跑指定检查。合法值只有 junk-tags / broken-links / empty-notes / missing-type / stale；回执会列出**实际运行**的检查，无法识别的名字会被明确报出来（不会静默当成「库很干净」）"
			},
			staleAfterDays: {
				type: "integer",
		...
		output: { render: (_args, value) => {
			...
			if (value.unknownChecks.length > 0) lines.push(`⚠️ 忽略了无法识别的检查名：${value.unknownChecks.join(", ")}——合法值只有 ${LINT_CHECKS.join(" / ")}`);
			if (value.issues.length === 0) lines.push(value.checks.length === 0 ? "没有运行任何检查（请求的检查名全部无法识别）。" : "没有发现问题。");
		...
		execute: async (args) => {
			...
			const requested = Array.isArray(args.checks) ? args.checks.filter((c) => typeof c === "string" && c.trim().length > 0).map((c) => c.trim()) : [];
			const unknownChecks = requested.filter((c) => !LINT_CHECKS.includes(c));
			const checks = requested.length > 0 ? LINT_CHECKS.filter((c) => requested.includes(c)) : [...LINT_CHECKS];
			const want = (kind) => checks.includes(kind);
			const items = await wiki.list(void 0, true);

...
const SEED_DEFS = [
	...
	defineSeed({
		id: "publish-spec",
		title: "发布元数据规范",
		description: "「发布元数据规范」文档（Markdown）：约定用 pub-state / pub-platform / pub-wechat-* 等自定义字段记录每篇笔记的对外发布状态（已发/未发/不可发），以及 no-publish 标签约定；供发布流程与 agent 判断「这篇发过没有、能不能发」使用。随插件分发，换机器无需重新约定。",

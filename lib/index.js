...
function cleanTiddler(t) {
	...
	if (typeof nested === "object" && nested !== null && !Array.isArray(nested)) for (const [k, v] of Object.entries(nested)) {
		if (CLEAN_SKIP_FIELDS.has(k) || v === void 0) continue;
		out[k] = v;
	}
	return out;
}
...
*/
function buildWriteTiddler(title, text, existing, tags, fields) {
	...
	if (tags !== void 0) {
		...
		const finalTags = finalTagsForWrite(title, void 0, []);
		if (finalTags.length > 0) tiddler.tags = finalTags;
	}
...
function assertNoConflict(title, existing, expected) {
	...
	throw new Error(`写入冲突：tiddler「${title}」在你读取之后已被改动（当前 revision=${currentRevision ?? "?"} modified=${toIsoDateString(existing.modified) ?? "?"}；期望 revision=${expected.expectedRevision ?? "?"} modified=${expected.expectedModified ?? "?"}）。请重新 tiddlywiki_get 获取最新内容后重试；确认要用你的版本覆盖时可传 force: true。`);
}
/**
	...
	register(defineTool({
		...
		execute: async (args) => {
			...
			if (existing !== void 0) await wiki.put({
				...
				const tags = finalTagsForWrite(title, void 0, Array.isArray(args.tags) ? args.tags.filter((t) => typeof t === "string" && t.trim().length > 0) : []);
				const tiddler = {
					title,
					text: next
				};
	...
	register(defineTool({
		...
		parameters: {
			...
			checks: {
				type: "array",
				items: { type: "string" },
				description: "可选：只跑指定检查（junk-tags / broken-links / empty-notes / missing-type / large-binary）"
			}
		},
		output: {
			schema: { type: "json" },
		...
		execute: async (args) => {
			...
			if (want("missing-type")) {
				...
				for (const t of items) {
					...
					if (type === "text/markdown" || type === "text/vnd.tiddlywiki") continue;
					const text = t.text ?? "";
				...
				if (count > 0) issues.push({
					kind: "missing-type",
					count,
					hint: "正文像 Markdown 但 type 不是 text/markdown，TW 会按 wikitext 渲染；用 fields.type 明确内容类型",
					samples
				});
			}
			return {
			...
			try {
				...
				if (seedClient !== void 0) {
					...
					if (needsRestartAfterSeeds(results)) {
						...
						await server.restart();
					} else if (pluginAdded) await server.restart();
				}
			} catch (err) {

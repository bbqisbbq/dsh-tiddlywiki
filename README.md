...
- **v0.26.7**（2026-09-26）：**修「DSH 桌面版里 TW 面板没有『添加条目』按钮」——嵌入式 TW 被渲染在自定义 scheme 上，于是它根本没有同步器**。现象（用户报障，桌面版）：TW 面板能看能搜，但页面控件里 **添加条目 / 添加日志 / 添加图片 / 导入 / 条目管理器** 整排不见，单条笔记的 **编辑 / 克隆 / 删除** 也不见，像装了个只读版 TW。**根因不在本插件，而在 TW 内核的一处协议判定 + 桌面版新增的渲染 scheme**：TiddlyWiki 自己的 TiddlyWeb sync adaptor 只在**文档协议以 `http` 开头**时才导出自己（`plugins/tiddlywiki/tiddlyweb/tiddlywebadaptor.js`：`if($tw.browser && document.location.protocol.substr(0,4) === "http")`）；而桌面版用自定义 scheme `dsh-app:` 渲染界面（已在本机运行中的 Electron 渲染进程命令行里核实 `--standard-schemes=dsh-app --secure-schemes=dsh-app …`），于是 iframe 里的 TW **没有 syncadaptor → 没有 `$tw.syncer`**：`$:/status/IsReadOnly` 永远没人写，TW 自己的只读样式表 `$:/plugins/tiddlywiki/tiddlyweb/readonly`（`<$reveal … default="yes">`，**状态未知即当只读**）就把那批按钮 `display:none` 掉；同时 saver 退化成本地 `dirtyTracking`，**在那儿做的编辑根本存不回 wiki**。实测复现（不依赖桌面版）：把同一份 wiki 页面用 `file://` 打开（同样非 http），＋ 同样消失——所以这不是「按钮坏了」，而是「这个文档里 TW 没有同步器」。**修复思路：让嵌入式 TW 跑在真正的 http origin 上**。① 宿主 `/status` / `/edit` / `/session/summary` 在原有相对字段旁，新增**绝对孪生字段** `twProxyAbsolute` / `twUrlAbsolute`（`absoluteHostBase()`，由请求 `Host` 头推导；脏头一律拒绝 `/^[A-Za-z0-9.\-[\]:]+$/`，避免把路径/凭据拼进回给浏览器的 URL）；② 客户端新增 `resolveTwUrl()`（`endpoints.ts`）——**页面本身是 http(s) 时继续走相对路径**（loopback / 局域网 / Tailscale / 域名 / HTTPS 行为一字不变，绝不把 https 页面改源），**只有页面不是 http(s) 时**才改用宿主回环 http 基址，于是桌面版 iframe 里 TW 的协议变成 `http:`，sync adaptor 正常加载、同步器回来、按钮与保存一起复原；③ 随之而来的跨源：桌面版下 TW frame 与宿主页面不同源，`tw-frame.ts` 的 hash 导航把「读跨源 `$tw` 抛 SecurityError」包进 try/catch 并直接走整页兜底（不再冒泡、链接不丢），`theme-sync` 本就是 best-effort 跨源静默。守门：新增 `scripts/verify-tw-origin.mjs`（7 条：Host 头推导 / 脏头拒绝 / http(s) 忽略绝对基址 / `dsh-app:`+`file:` 采用绝对基址 / 老服务端不回归 / 两处接线断言 / 跨源兜底源码断言），`verify-frame-surface` 再增 2 条行为单测（`dsh-app:` 页面必须把 frame 指到宿主 http 基址、且 http 页面仍用同源相对地址；跨源 frame 读 `$tw` 抛错时必须整页兜底）。**注意**：这修的是「TW 被非 http(s) 文档嵌入」；「在浏览器里直连 TW 子端口（`http://127.0.0.1:<port>/`）」是另一回事——那样 TW 的 API 基址会被指到 DSH 代理前缀而 404，同样显示成只读，请一律用 DSH 同源入口（面板 / 右侧栏知识库 tab / `<DSH origin>/dsh-tiddlywiki/tw/`）。

...
| 📝 **快速笔记** | 右下角「知识库」悬浮按钮 **或聊天输入框上方的快捷按钮**（`ui.showQuickNoteDock`，与 todo/cost-meter/goal 等插件内容同槽位纵向排列、不重叠；按钮右缘实时贴齐输入框）→ **两种打开方式可切换**（`ui.quickNoteMode`，默认**原生模式**）：**原生模式**点击直接弹出 **TW 原生编辑页**（独立可拖动/缩放小窗，草稿自动续写，✕/Esc/再点按钮均可关闭）；**卡片模式**为原有快速笔记卡片：**CodeMirror 6** Markdown 编辑器（语法高亮 + 撤销/重做）、文件上传、多选/自动补全 tag、**草稿自动保存（刷新不丢）**、**「🕘 最近」一键载入旧笔记**，Ctrl+Enter 保存；**从按钮打开时卡片在按钮上方弹出、按住标题栏可自由拖动、再次点按钮可收起**；**卡片只允许点右上角 ✕ 或输入框按钮关闭**（点页面其他位置不会误关）；可整体隐藏 |
...
- **v0.16.17**（2026-09-07）：**新：点击「快速笔记」可直达 TW 原生编辑页（`ui.quickNoteMode` 可切换）+ 修「✏️ 在 TW 中编辑」弹窗关不掉**。① **原生模式（默认）**：点击输入框上方/「知识库」菜单里的「快速笔记」不再先弹 Markdown 卡片，而是**直接弹出 TW 原生编辑页**（独立小窗）：有未保存草稿自动继续编辑（标题/正文/tag 全恢复），没有则新建「时间戳标题 + 默认 tag」空草稿；在 TW 里编辑、保存/取消都是 TW 原生行为。设置页「常规配置」新增「点击快速笔记的打开方式」下拉（原生编辑器 / Markdown 卡片，`ui.quickNoteMode: native|card`，默认 native；改为 card 即恢复旧卡片体验，卡片全部功能保留）。② **修复弹窗 ✕ 关不掉**：根因是弹窗标题栏的拖动 handler 对**任何** pointerdown 都 `preventDefault()`——包括点在 ✕ 上（Pointer Events 规范里取消的 pointerdown 会吞掉派生的 click 事件，✕ 的 click 监听永远不触发，弹窗只能靠刷新页面关掉；快速笔记卡片自己的拖动 handler 有排除关闭按钮的守卫，弹窗漏了）。修复：拖动 handler 排除 ✕（与卡片一致），另加 **Esc 键关闭**；原生模式下再次点击输入框按钮也可收起弹窗。host 改动：`npm run build:host` + **重启 dsh web**；客户端改动刷新页面即生效，设置页改完即时生效。

- **v0.16.16**（2026-09-07）：**修：会话「知识库」Tab 整页显示 wikitext 源码、链接点不动——根因是误入了 TW 的 ✏️ 编辑草稿视图**。汇总条目正文是 TW wikitext，数据/类型/渲染链路本身都是好的（服务端 render 实测输出含可点击 `<a>`）；但一旦在该条目上点过 ✏️ 编辑，`Draft of '…'` 草稿会**继承全文**并以编辑框（textarea）呈现——看起来就是「没正确渲染、显示的是源码」，草稿还会被浏览器端 TW 同步回流服务端/落盘（日志里可看到 `Draft of '$:/temp/dsh/session-summary/…'` 的 save/delete 任务）。修复（纯客户端）：注入后对汇总条目做**三层防误编辑**——① 清理每次挂载时残留的该汇总草稿（含从故事列表移除）；② 在 iframe 的 `wiki.addTiddler` 层吞掉 `draft.of` 指向汇总的新草稿创建（幂等，每个 window 只包装一次）；③ 禁用该 tiddler 帧的 ✏️ 编辑按钮（本地化 tooltip 定位，不依赖固定语言）。**顺带修复**：TW 面板 iframe 首帧侧边栏布局测量——load 后延时驱动 `$tw.rootWidget.refresh` 重渲染 `$:/core/ui/PageTemplate/sidebar` 并派发一次 `resize`（5.4.1 没有 `wiki.refreshTiddler`，用等价的 change 信号；多次延时兜底 boot 时序，theme-sync 同款 load 钩子模式，卸载时清定时器）。刷新页面即生效。
...
**📝 快速笔记** — 两个入口，两种打开方式（`ui.quickNoteMode`，设置页可切换）：
- **原生模式（默认，v0.16.17）**：点击入口**直接弹出 TW 原生编辑页**（独立小窗，可拖动/缩放）——立刻可用 TW 自己的 Markdown 编辑、标签、保存/取消；有未保存草稿会自动继续编辑它，没有则新建「时间戳标题 + 默认 tag」的空草稿；再次点击输入框上方的按钮可收起弹窗，弹窗右上角 ✕ / Esc 也能关（v0.16.17 修复了 ✕ 关不掉的问题）；
- **卡片模式（`ui.quickNoteMode: "card"`）**：保留原有 Markdown 卡片体验（下面列出的功能全在卡片模式里）；
...
- 卡片模式里点「**✏️ 在 TW 中编辑**」→ 保存后弹出**独立小窗**（可拖动/缩放）加载 TW 原生编辑器编辑该条。
...
- 汇总条目**不可编辑**（v0.16.16）：它是 volatile 临时内容、每次自动重新生成，编辑毫无意义——若误入 TW 的 ✏️ 编辑草稿视图，整页会呈现为 wikitext 源码（链接点不动），草稿还会被浏览器端同步回流服务端。插件在注入后自动**清理残留草稿**、**吞掉指向汇总的新草稿创建**并**禁用 ✏️ 编辑按钮**；
...
| 常规配置 | 快速笔记默认 tag、**点击「快速笔记」的打开方式（原生编辑器 / Markdown 卡片，`ui.quickNoteMode`）**、git 自动 commit/防抖/远端/分支、ui 开关（快速笔记/输入框上方快捷按钮/面板状态/同步按钮——分别控制对应入口）、**侧边栏 TW 入口显示名称**、**跟随 DSH 主题开关 + 深色 palette**——改了什么保存什么 |
...
- id: dsh-tiddlywiki
  ...
  config:
    ...
    ui:
      ...
      quickNoteMode: "native"       # 点击「快速笔记」的打开方式：native=直达 TW 原生编辑页（默认）；card=Markdown 卡片

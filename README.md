...
| 📝 **快速笔记** | 右下角「知识库」悬浮按钮 **或聊天输入框上方的快捷按钮**（`ui.showQuickNoteDock`，与 todo/cost-meter/goal 等插件内容同槽位纵向排列、不重叠；按钮右缘实时贴齐输入框）→ 快速笔记卡片：**CodeMirror 6** Markdown 编辑器（语法高亮 + 撤销/重做）、文件上传、多选/自动补全 tag、**草稿自动保存（刷新不丢）**、**「🕘 最近」一键载入旧笔记**，Ctrl+Enter 保存；**从按钮打开时卡片在按钮上方弹出、按住标题栏可自由拖动**；**卡片只允许点右上角 ✕ 关闭**（点页面其他位置不会误关）；可整体隐藏 |

 ...
 *
 * v0.16.25: the bookmarklet opens a PICKER overlay — editable title / selection
 * text plus a thumbnailed image checklist (og:image auto-marked as 封面 and
 * preselected); chosen images are downloaded by the bridge and stored as
 * binary attachment tiddlers (type image/*, base64 text), embedded in the note
 * via `[img[Title]]`.
 *
 * v0.16.26 (fix): the overlay fields are resolved through `p.querySelector`
 * (scoped to the overlay) instead of document.getElementById, and their values
 * are set AFTER `document.body.appendChild(ov)` — detach-timing + page-id
 * collision safe (regression guarded by verify-clip-bridge.mjs static checks).
 *
 * The bookmarklet is ONE long line by design (bookmarks-bar URL fields; no
 * backticks / ${} to survive the fence + the template literal).
 *

 ...
 *
 * v0.16.25: the bookmarklet now opens a PICKER overlay — editable title /
 * selection text plus a thumbnailed image checklist (og:image auto-marked as
 * 封面 and preselected); chosen images are downloaded by the bridge and stored
 * as binary attachment tiddlers (type image/*, base64 text), embedded in the
 * note via `[img[Title]]`. The bookmarklet is ONE long line by design
 * (bookmarks-bar URL fields; no backticks / ${} to survive the fence + the
 * template literal).
 *
